...
*/
Index *Heap_Sort_Heapify(Index peak[], long head, long tail, Compare comp) {
    Index swap;
    // long head = 0;
    long left = head * 2 + 1;
    while (left < tail) {
...
Index *Heap_Sort_BT(Index index[], long leng, Compare comp) {
    ...
    for (long idx = leng / 2 - 1; 0 <= idx; idx -= 1) {
        Heap_Sort_Heapify(&index[0], idx, leng - 1, comp);
    }

    for (long i = leng - 1; 0 < i; i -= 1) {
        swap = index[0], index[0] = index[i], index[i] = swap;
        // Heap_Sort_Heapify(&index[0], i - 1, comp);
        Heap_Sort_Heapify(&index[0], 0, i - 1, comp);
        Print_Index(index, leng);
    }
    return index;
}
