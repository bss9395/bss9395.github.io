...
public class Captcha {
    static public void _Captcha(int _count) {
        ...
        BufferedImage image = new BufferedImage((int)(max_width * (_count * 2 + 1)), (int)(max_height * 3), BufferedImage.TYPE_INT_ARGB);    // 【0.5 + 0.5 + 1.0 + 0.5 + 0.5】
        ...
        for(int i = 0; i < code.length(); i += 1) {
            ...
            transform.rotate(Generator._Generate_Double(-75.0, +75.0) / 180.0 * Math.PI);    // 以基线左下角为原点
            transform.translate(-bounds[i].getWidth() * 0.5, 0.0);
            graphics.setTransform(transform);
            graphics.drawString("" + code.charAt(i), 0, 0);    // 以基线左下角为原点
        }
