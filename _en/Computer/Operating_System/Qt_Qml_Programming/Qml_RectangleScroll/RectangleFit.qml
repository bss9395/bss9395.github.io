...
Rectangle {
    ...
    Component.onCompleted: {
        // console.debug(`rectangle:onCompleted, width = ${width}, height = ${height}, anchors = ${JSON.stringify(anchors)}`)
        if(0 < width) {
            initedWidth  = true
        }
        if(0 < height) {
    ...
    function slot_updateChildrenRect() {
        ...
        // console.debug(`rectangle.width = ${maxiX - miniX}, rectangle.height = ${maxiY - miniY}`)
        if(initedWidth === false) {
