/* Login.h
Author: BSS9395
Update: 2022-02-13T22:40:00+08@China-Guangdong-Shenzhen+08
Design: Canvas
Encode: UTF-8
System: Qt 5.14.2
*/
...
public:
    struct Datum {
        // data saved on Windows regedit.exe, /计算机/HKEY_CURRENT_USER/SOFTWARE/BSS9395/画布/
...
public:
    explicit Login(QWidget *parent = nullptr, iptr *result = nullptr)
        ...
        _ui->CB_Password->lineEdit()->setEchoMode(QLineEdit::Password);
        ...
        QObject::connect(_ui->CB_Username, &QComboBox::editTextChanged, [this]() -> void {
            ...
            if(_reading == false) {    // note: signal shielding guard.
...
public:
    ...
    void Sign_In() {
        ...
        if(Setting::_Username == datum._username) {
            // if(Setting::_Encrypted == datum._encrypted) {
            if(Encrypt(Setting::_Password) == datum._encrypted) {
                (*_return) = QDialog::Accepted;
                this->accept();
        ...
        _tried -= 1;
        _ui->CB_Username->lineEdit()->setPlaceholderText(QString("%1次试错机会剩余%2次").arg(Setting::_Tried).arg(_tried));
        if(_tried <= 0) {
            QString caption = QString("错误提示");
            QString label = QString("%1次试错机会剩余0次").arg(Setting::_Tried);
            QMessageBox::critical(this, caption, label);
            (*_return) = QDialog::Rejected;
            this->reject();
            return ;
        }
    }
