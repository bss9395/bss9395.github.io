...
//    Tcl_Init(interp);
//    Tk_Init(interp);
//    Tcl_Eval(interp, "tk scaling");
//    const char* tk_scaling = Tcl_GetStringResult(interp);
//    fprintf(stdout, "tk scaling = %s\n", tk_scaling);
//
//    Tk_Window tkwin = Tk_MainWindow(interp);    // 创建 Tk 主窗口
//    Tk_MakeWindowExist(tkwin);                  // 强制创建底层窗口
...
//    UINT dpiY = 96;
//    GetDpiForMonitor((HMONITOR)monitor, MDT_EFFECTIVE_DPI, &dpiX, &dpiY);
//    int dpiWindow = GetDpiForWindow(hwnd);
//    fprintf(stdout, "dpi = (%d, %d, %d, %d), scale = (%f, %f, %f, %f)\n", dpiSystem, dpiX, dpiY, dpiWindow, (double)dpiSystem / 96, (double)dpiX / 96, (double)dpiY / 96, (double)dpiWindow / 96);
//}
//
//int main() {
//    // _Screen();
