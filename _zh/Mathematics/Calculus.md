...
| $\dfrac{\mathrm{d}}{\mathrm{d} x} \left[ \dfrac{f(x)}{g(x)} \right] $ | $\dfrac{\dfrac{\mathrm{d}f(x)}{\mathrm{d} x} · g(x) - f(x) · \dfrac{\mathrm{d} g(x)}{\mathrm{d} x}}{g^2(x)}$ | $\lim\limits_{x_t⇝x} \dfrac{\dfrac{f(x_t)}{g(x_t)} - \dfrac{f(x)}{g(x)}}{x_t - x} = \lim\limits_{x_t⇝x} \dfrac{\dfrac{[f(x_t) - f(x)] · g(x)}{x_t - x} - \dfrac{f(x) · [g(x_t) - g(x)]}{x_t - x}}{g(x_t) · g(x)} \mathop{===}\limits^{g(x)≠0} \dfrac{\dfrac{\mathrm{d}f(x)}{\mathrm{d}x} · g(x) - f(x) · \dfrac{\mathrm{d} g(x)}{\mathrm{d} x}}{g^2(x)}$ |
