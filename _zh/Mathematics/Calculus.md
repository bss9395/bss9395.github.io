...
点$x_0$的闭邻域$\mathrm{B}_{X}(x_0, δ)$由属于区域$X$中的点构成，点$x_0$未必属于区域$X$。点$x_0$的任意闭邻域$\mathrm{B}_{X}(x_0)$与其任意补邻域$\mathrm{B}_{X}^{¬}(x_0)$之间没有交集。
...
|     闭邻域 | $\mathrm{B}_{X}(x_0, δ)$    | $∀x∈X; [ x∈\mathrm{B}_{X}(x_0, δ) ] ⇔ [ 0 < | x - x_0 | ≤ δ ]$ |                |
|     补领域 | $\mathrm{B}_{X}^{¬}(x_0,δ)$ | $∀x∈X; [x∉\mathrm{B}_{X}^{¬}(x_0, δ)] ⇔ [0≤|x-x_0|≤δ]$       |                |
|     闭包集 | $\fbox{X}$                  | $∀x_0; \left[ x_0∈\fbox{X} \right] ⇔ [ ∀δ>0; \mathrm{B}_{X}(x_0, δ) ≠ 𝟘 ]$ | $\fbox{X}⊇X$   |
|     聚敛点 | $x_0∈\fbox{X}$              | $∀δ>0; \mathrm{B}_{X}(x_0, δ) ≠ 𝟘$                           |                |
|     孤立点 | $x_0∈X$                     | $∃δ>0; \mathrm{B}_{X}(x_0, δ) = 𝟘$                           |                |
|     边界点 | $x_0∈\fbox{X}$              | $∀δ>0; [ \mathrm{B}_{X}(x_0,δ) ≠ 𝟘 ] ∧ [ \mathrm{B}_{X}^{¬}(x_0,δ) ≠ 𝟘 ]$ |                |
|     内敛点 | $x_0∈X$                     | $∃δ>0; \mathrm{B}_{X}(x_0, δ) ⊆ X$                           |                |
|   全闭区域 | $[x_α, x_β]$                | $x_α∈[x_α,x_β] ∧ x_β∈[x_α,x_β]$                              | $X = \fbox{X}$ |
|   全开区域 | $(x_α, x_β)$                | $x_α∉(x_α,x_β) ∧ x_β∉(x_α,x_β)$                              | $X ≠ \fbox{X}$ |
...
|           $∀x∈[x_α, x_β]; | f (x) | ≤ ε$ | $⇔$  | $\sup\limits_{x∈[X_0,X_1]} | f (x) | ≤ ε$     |                                                   |         $∃x∈[x_α, x_β]; | f (x) | > ε$ | $⇔$  | $\sup\limits_{x∈[X_0,X_1]} | f (x) | > ε$     |
| --------------------------------------------: | :--: | :-------------------------------------------- | :-----------------------------------------------: | --------------------------------------------: | :--: | :-------------------------------------------- |
|         $∀x∈[x_α,x_β]; | f (x) | < ε$ | $⇔$  | $\sup\limits_{x∈[X_0,X_1]} | f (x) | < ε$     |                                                   |          $∃x∈[x_α,x_β]; | f (x) | ≥ ε$ | $⇔$  | $\sup\limits_{x∈[X_0,X_1]} | f (x) | ≥ ε$     |
|          $∀x∈(x_α,x_β); | f (x) | ≤ ε$ | $⇔$  | $\sup\limits_{x∈(X_0,X_1)} | f (x) | ≤ ε$     |                                                   |          $∃x∈(x_α,x_β); | f (x) | > ε$ | $⇔$  | $\sup\limits_{x∈(X_0,X_1)} | f (x) | > ε$     |
|          $∀x∈(x_α,x_β); | f (x) | < ε$ | $⇒$  | $\sup\limits_{x∈(X_0,X_1)} | f (x) | ≤ ε$     | $\rlap{≡≡≡≡≡≡≡≡≡}\sup\limits_{x⇝X_0} |f (x)| = ε$ |          $∃x∈(x_α,x_β); |f(x)| ≥ ε$ | $⇒$  | $\sup\limits_{x∈(X_0,X_1)} | f (x) | ≥ ε$     |
|                                               |      |                                               |                                                   |                                               |      |                                               |
...
| :--: | :----------------------------------------------------------- | :-------------------------------------------------: | :----------------------------------------------------------- |
| $⇓$  | $∀ε>0;∃δ>0;∀x∈\mathrm{B}(x_0,δ); |f(x) - f_{x_0}| ≤ ε$       |                                                     |                                                              |
| $⇓$  | $∀ε>0;∃δ>0;∀x_1,x_2∈\mathrm{B}(x_0, δ); |f(x_1) - f(x_2)| ≤ |f(x_1) - f_{x_0}| + |f(x_2) - f_{x_0}| ≤ 2·ε_0 = ε$ | $\mathop{⇋}\limits_{δ≥\frac{1}{N}}^{δ≤\frac{1}{N}}$ | $∀ε>0;∃N∈ℕ;∀n,m≥N; x_n,x_m∈\mathrm{B}\left(x_0,\frac{1}{N}\right) ⇒ |f(x_n) - f(x_m)| ≤ ε$ |
| $⇓$  | $∀ε>0;∃δ>0;∀x_1,x_2∈\mathrm{B}(x_0,δ); |f(x_1) - f(x_2)| ≤ ε$ |                         $⇔$                         | $∀ε>0;∃δ>0; \sup\limits_{x_1,x_2∈\mathrm{B}(x_0,δ)} |f(x_1) - f(x_2)| ≤ ε$ |
| $⇓$  | $∀ε>0;∃δ>0;∀x,x_1∈\mathrm{B}(x_0,δ); f(x_1) - ε ≤ f(x) ≤ f(x_1) + ε$ |                                                     |                                                              |
| $⇓$  | $f_{x_0} ⇜ \lim\limits_{δ⇝0} \inf\limits_{x∈\mathrm{B}(x_0,δ)} f(x) = \lim\limits_{δ⇝0} \sup\limits_{x∈\mathrm{B}(x_0,δ)} f(x)$ |                                                     |                                                              |
| $⇓$  | $∀ε>0;∃δ>0;∀x∈\mathrm{B}(x_0,δ); |f(x) - f_{x_0}| ≤ ε$       | $\mathop{⇋}\limits_{δ≥\frac{1}{N}}^{δ≤\frac{1}{N}}$ | $∀ε>0;∃N∈ℕ;∀n≥N; x_n∈\mathrm{B}\left(x_0,\frac{1}{N}\right) ⇒ |f(x_n) - f_{x_0}| ≤ ε$ |
...
| $⇓$  | $∀ζ>0;∃ε>0;∀y; y∈\mathrm{B}(y_0,ε) ⇒ f(y)∈\mathrm{B}(f_{y_0},ζ)$ |                  |
| :--: | :----------------------------------------------------------- | :--------------- |
| $⇓$  | $∀ε>0;∃δ>0;∀x; x∈\mathrm{B}(x_0,δ) ⇒ g(x)∈\mathrm{B}(y_0,ε)$ | $g_{x_0} = y_0$  |
| $⇓$  | $∀ζ>0;∃ε>0;∃δ>0;∀x; x∈\mathrm{B}(x_0,δ) ⇒ g(x)∈\mathrm{B}(y_0,ε) ⇒ f(g(x))∈\mathrm{B}(f_{y_0},ζ)$ | $g(x) ≠ g_{x_0}$ |
| $⇓$  | $∀ζ>0;∃δ>0;∀x∈\mathrm{B}(x_0,δ); f(g(x))∈\mathrm{B}(f_{y_0},ζ)$ |                  |
...
$w_{X}^{f}(x_0,δ) ≡ \sup\limits_{u,v∈\mathrm{\fbox{B}}_{X}(x_0,δ)} |f(u) - f(v)| = \left[ \sup\limits_{u∈\mathrm{\fbox{B}}_{X}(x_0,δ)} f(u) - \inf\limits_{v∈\mathrm{\fbox{B}}_{X}(x_0,δ)} f(v) \right]$
...
| $w^{f}(x_0) ≠ 0$   | $∃ε>0;∀δ>0; \sup\limits_{u,v∈\mathrm{\fbox{B}}_{X}(x_0,δ)} |f(u) - f(v)|>ε$ | $∃ε>0;∀δ>0;∃u,v∈\mathrm{\fbox{B}}_{X}(x_0,δ); |f(u) - f(v)|>ε$ | $∃ε>0;∀δ>0;∃u,v∈X; |u,v-x_0|≤δ ∧ |f(u) - f(v)|>ε$   |
| :----------------- | :----------------------------------------------------------- | :----------------------------------------------------------- | :-------------------------------------------------- |
| $$w^{f}(x_0) = 0$$ | $∀ε>0;∃δ>0; \sup\limits_{u,v∈\mathrm{\fbox{B}}_{X}(x_0,δ)} |f(u) - f(v)| ≤ ε$ | $∀ε>0;∃δ>0;∀u,v∈\mathrm{\fbox{B}}_{X}(x_0,δ); |f(u) - f(v)| ≤ ε$ | $∀ε>0;∃δ>0;∀u,v∈X; |u,v-x_0|≤δ ⇒ |f(u) - f(v)| ≤ ε$ |
...
| :--: | ------------------------------------------------------------ | ------------------------------------- |
| $⇓$  | $∀ε_u>0;∃δ_u>0;∀ε_v>0;∃δ_v>0;∀ε>0;∃δ>0;∀u,v∈\mathrm{\fbox{B}}(x_0,δ=\min\lbrace δ_u,δ_v \rbrace); |f(u) - f(v)| ≤ |f(u) - f(x_0)| + |f(v) - f(x_0)| ≤ ε_u + ε_v = ε$ |                                       |
| $⇓$  | $∀ε>0;∃δ>0;∀u,v∈\mathrm{\fbox{B}}(x_0,δ); |f(u) - f(v)| ≤ ε$ | $\lim\limits_{δ⇝0} w^{f} (x_0,δ) ⇝ 0$ |
| $⇓$  | $∀ε>0;∃δ>0;∀x∈\mathrm{B}(x_0,δ); |f(x) - f(x_0)| ≤ ε$        |                                       |
...
| $⇓$  | $∀ζ>0;∃ε>0;∀y; y∈\mathrm{B}(y_0,ε) ⇒ f(y)∈\mathrm{B}(f(y_0),ζ)$ |                |
| :--: | :----------------------------------------------------------- | :------------- |
| $⇓$  | $∀ε>0;∃δ>0;∀x; x∈\mathrm{B}(x_0,δ) ⇒ g(x)∈\mathrm{B}(y_0,ε)$ | $g(x_0) = y_0$ |
| $⇓$  | $∀ζ>0;∃ε>0;∃δ>0;∀x; x∈\mathrm{B}(x_0,δ) ⇒ g(x)∈\mathrm{B}(y_0,ε) ⇒ f(g(x))∈\mathrm{B}(f(y_0),ζ)$ |                |
| $⇓$  | $∀ζ>0;∃δ>0;∀x∈\mathrm{B}(x_0,δ); f(g(x))∈\mathrm{B}(f(y_0),ζ)$ |                |
