...
函数$f(x)$在点$x_0$处振幅$w^{f}(x_0,δ)$。若函数$f(x)$在点$x_0$处未定义，则其在该点处振幅$w^{f}(x_0,δ)$可视为无穷大。

$w^{f}(x_0,δ) ≡ \sup\limits_{u,v∈\mathrm{\fbox{B}}(x_0,δ)} |f(u) - f(v)| = \left[ \sup\limits_{|u-x_0|≤δ} f(u) - \inf\limits_{|v-x_0|≤δ} f(v) \right]$

对于任意间断点$x_0$，其所在任意区间内的振幅均不为零。

| $\lim\limits_{δ⇝0} w^f (x_0,δ) \not⇝ 0$ | $∃ε>0;∀δ>0; \sup\limits_{u,v∈\mathrm{B}(x_0,δ)} |f(u) - f(v)|>ε$ | $∃ε>0;∀δ>0;∃u,v∈\mathrm{B}(x_0,δ); |f(u) - f(v)|>ε$ |
| :-------------------------------------- | :----------------------------------------------------------- | :-------------------------------------------------- |
|                                         |                                                              |                                                     |
...
| $⇓$  | $∀ε_1>0;∃δ>0;∀u∈\mathrm{B}(x_0,δ); |f(u) - f(x_0)|≤ε_1$      | $\lim\limits_{x⇝x_0} f (x) ⇝ f (x_0)$ |
| :--: | :----------------------------------------------------------- | :------------------------------------ |
| $⇓$  | $∀ε>0;∃δ>0;∀u,v∈\mathrm{B}(x_0,δ); |f(u) - f(v)| ≤ |f(u) - f(x_0)| + |f(v) - f(x_0)| ≤ ε_1 + ε_2 = ε$ |                                       |
| $⇓$  | $∀ε>0;∃δ>0;∀u,v∈\mathrm{B}(x_0,δ); |f(u) - f(v)| ≤ ε$        | $\lim\limits_{δ⇝0} w^{f} (x_0,δ) ⇝ 0$ |
