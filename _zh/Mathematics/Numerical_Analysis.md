...
Topic :  收敛极限
Title :  数值分析
Update:  2021-05-12T02:21:00+08@中国-广东-湛江+08
Author:  璀璨星辰
Link  :
---
...
| $[n = 2 · l + 0] ∧ [{^n}f(x_0) < 0]$                      |                             $⇔$                              | $[f(x_0^{-}) - l(x_0^{-}) < 0] ∧ [f(x_0^{+}) - l(x_0^{+}) < 0]$ | $x_0$为相切点，$f(x)$在切线$l(x)$下方。           |
| $[n = 2 · l + 0] ∧ [{^n}f(x_0) > 0]$                      |                             $⇔$                              | $[f(x_0^{-}) - l(x_0^{-}) > 0] ∧ [f(x_0^{+}) - l(x_0^{+}) > 0]$ | $x_0$为相切点，$f(x)$在切线$l(x)$上方。           |
| $[n = 2 · l + 1] ∧ [{^n}f(x_0) < 0]$                      |                             $⇔$                              | $[f(x_0^{-}) - l(x_0^{-}) > 0] ∧ [f(x_0^{+}) - l(x_0^{+}) < 0]$ | $x_0$为相交点，$f(x)$于切线$l(x)$向下。           |
| $[n = 2 · l + 1] ∧ [{^n}f(x_0) > 0]$                      |                             $⇔$                              | $[f(x_0^{-}) - l(x_0^{-}) < 0] ∧ [f(x_0^{+}) - l(x_0^{+}) > 0]$ | $x_0$为相交点，$f(x)$于切线$l(x)$向上。           |
