...
Topic :  收敛极限
Title :  离散数学
Update:  2021-04-15T23:07:00+08@中国-广东-湛江+08
Author:  璀璨星辰
Link  :
---
...
若集合$S$满足$\mathrm{ord} S ≈ \mathrm{o}[ϰ_1]$，则称集合$S$为可数集，否则称集合$S$为不可数集，因此有限集为可数集。

自然数集/整数集/有理数集为可数集，无理数集/实数集/复数集为不可数集。
...
|        $\mathrm{Θ}[ϰ_0] ≈ ϰ_0$        | $<$  |        $\mathrm{O}[ϰ_0] ≈ 2 · ϰ_0$        | $<$  |       $\mathrm{O}[ϰ_0^2] ≈ 2 · ϰ_0^2$        | $<$  |  $\mathrm{O}\left[ϰ_0^{4}\right] ≈ 2 · ϰ_0^{4}$  | $≪$  |    $\mathrm{O}[ϰ_0^{ϰ_0}] ≈ 2 · ϰ_0^{ϰ_0}$    | $=$  | $\mathrm{O}[ϰ_1] ≈ 2 · ϰ_1$ | $<$  |     $\mathrm{O}[ϰ_1^2] ≈ 4 · ϰ_1^2$      |
...
| 0维  | $\mathrm{fld} ●  = \left[ \dfrac{\mathrm{fld} ℝ}{\mathrm{ord} ℝ} \right]^0 = э^0 = \dfrac{ϰ_0^0}{ϰ_1^0} = 1$ | $\mathrm{fld} ℝ = 2 · ϰ_0$                | $\mathrm{ord} ℝ = 2 · ϰ_1$                |
| :--: | :----------------------------------------------------------- | :---------------------------------------- | :---------------------------------------- |
| 1维  | $\mathrm{fld} ● = \left[ \dfrac{\mathrm{fld} ℝ}{\mathrm{ord} ℝ} \right]^1 ≡ э^1 ≡ \dfrac{ϰ_0^1}{ϰ_1^1} = \mathrm{o}[1]$ | $\mathrm{fld}([0,1]) = 1$                 | $\mathrm{fld} ((0,1)) = 1$                |
...
可数集与零测集彼此等价，可数集强调集合中元素的数量，零测集强调集合中元素的度量。

任意零测集$\mathcal{O}$，必存在可数个区间$X_{i∈\mathcal{P}ℕ}$覆盖之，使得其可测度大小为零。

$∀\mathcal{O};∀ε_1>0;∃\lbrace X_{i∈\mathcal{P}ℕ} \rbrace; \left[ \mathcal{O} ⊆ \bigcup\limits_{i=0}^{\mathcal{P}ℕ} X_i \right] ∧ \left[ \mathrm{ord} \mathcal{O} ≤ \sum\limits_{i=0}^{\mathcal{P}ℕ} \mathrm{ord} X_i < ε_1 \right]$

可数个零测集$\mathcal{O}_i$的合集$\lbrace \mathcal{O}_{i∈\mathcal{P}ℕ} \rbrace$为零测集。

$∀\lbrace\mathcal{O}_{i∈\mathcal{P}ℕ}\rbrace;∀ε_3>0;∃\lbrace X_{ {i∈\mathcal{P}ℕ},{j∈\mathcal{P}ℕ} } \rbrace; \left[ \lbrace \mathcal{O}_{i∈\mathcal{P}ℕ} \rbrace ⊆ \bigcup\limits_{i=0}^{\mathcal{P}ℕ} \bigcup\limits_{j=0}^{\mathcal{P}ℕ} X_{i,j} \right] ∧ \left[ \mathrm{ord} \lbrace \mathcal{O}_{i∈\mathcal{P}ℕ} \rbrace = \sum\limits_{i=0}^{\mathcal{P}ℕ} \mathrm{ord} \mathcal{O}_i = \sum\limits_{i=0}^{\mathcal{P}ℕ} \sum\limits_{j=0}^{\mathcal{P}ℕ} \mathrm{ord} X_{i,j} ≤ \sum\limits_{i=0}^{∞⁺} \dfrac{1}{2^{i} } · \sum\limits_{j=0}^{∞⁺} \dfrac{ε_1}{2^{j} } ≤ \sum\limits_{i=0}^{∞⁺} \dfrac{ε_2}{2^i} = 2 · ε_2 = ε_3 \right]$
