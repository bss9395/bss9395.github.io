...
Topic:     顺其自然
Title:     公理化人类社会
Revised:   2019-01-12 09:19:00 +08 @ 中国-广东-湛江 +08
Authors:   璀璨星辰
Resources:
---
...
命题：有理数是稠密的。
证明⬇
对于任意有理数$\dfrac{p_1}{q_1}$与有理数$\dfrac{p_2}{q_2}$，若$\dfrac{p_1}{q_1} \lt \dfrac{p_2}{q_2}$，则$\dfrac{p_1}{q_1} \lt \dfrac{1}{2} \left(\dfrac{p_1}{q_1} + \dfrac{p_2}{q_2} \right) \lt \dfrac{p_2}{q_2}$，且$\dfrac{1}{2} \left(\dfrac{p_1}{q_1} + \dfrac{p_2}{q_2} \right)$是有理数。
即任意两个不同有理数之间至少存在一个有理数，因此任意两个不同的有理数之间存在无穷多个有理数。
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
推论：在实数中有唯一加法零元$0$。
若在实数中有两个不同的零元$0_1$与零元$0_2$，
则$0_1 = 0_1 + 0_2 = 0_2 + 0_1 = 0_2$。

推论：对于任意实数$x$，有唯一加法负元$(-x)$。
若对于任意实数$x$，有两个不同的负元$(-x)_1$和负元$(-x)_2$，
则$(-x)_1 = (-x)_1 + 0 = (-x)_1 + [x + (-x)_2] = [(-x)_1 + x] + (-x)_2 = 0 + (-x)_2 = (-x)_2 + 0 = (-x)_2$。

推论：在实数中有唯一乘法幺元$1$。

推论：对于任意实数$x$，有唯一乘法逆元$x^{-1}$。

定义：$x \lt y$，当且仅当$x \le y \land x \ne y$。
