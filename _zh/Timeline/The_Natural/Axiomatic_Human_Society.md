...
Topic:     顺其自然
Title:     公理化人类社会
Revised:   2019-01-13 06:14:00 +08 @ 中国-广东-湛江 +08
Authors:   璀璨星辰
Resources:
---
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Ⅰ 关联公理共8条：
Ⅰ1 对于任意两点，恒有一条直线与这两点相关联。
Ⅰ2 对于任意两点，至多有一条直线与这两点相关联。
...
Ⅴ 连续公理共2条：
Ⅴ1 [阿基米德公理] 已知线段AB和线段CD，存在数n使得在沿A到B的射线上，自A作首尾相接的n个线段CD，将越过B点。
...
推论：在实数中有唯一加法零元$0$。
推论：在实数中有唯一乘法幺元$1$。
证明：若在实数中有两个零元$0_1$与零元$0_2$，则$0_1 = 0_1 + 0_2 = 0_2 + 0_1 = 0_2$。
证明：若在实数中有两个幺元$1_1$与幺元$1_2$，则$1_1 = 1_1 × 1_2 = 1_2 × 1_1 = 1_2$。

推论：对于实数$x$，有唯一加法负元$(-x)$。
推论：对于实数$x ≠ 0$，有唯一乘法逆元$x^{-1}$。
证明：若对于实数$x$，有两个负元$(-x)_1$和负元$(-x)_2$，则$(-x)_1 = (-x)_1 + 0 = (-x)_1 + [x + (-x)_2] = [(-x)_1 + x] + (-x)_2 = 0 + (-x)_2 = (-x)_2 + 0 = (-x)_2$。
证明：若对于实数$x ≠ 0$，有两个逆元$(x^{-1})_1$和逆元$(x^{-1})_2$，则$(x^{-1})_1 = (x^{-1})_1 × 1 = (x^{-1})_1 × [x × (x^{-1})_2] = [(x^{-1})_1 × x] × (x^{-1})_2 = 1 × (x^{-1})_2 = (x^{-1})_2 × 1 = (x^{-1})_2$。

定义：减法是加法的负运算，对于实数$b$，有$a - b = a + (-b)$。
定义：除法是乘法的逆运算，对于实数$b ≠ 0$，有$a ÷ b = a × b^{-1}$。

推论：对于实数$x$，有$-(-x) = x$。
推论：对于实数$x ≠ 0$，有$(x^{-1})^{-1} = x$。
证明：对于实数$x$，有$x + (-x) = 0 \iff [x + (-x)] - (-x) = 0 - (-x) \iff x + [(-x) - (-x)] = - (-x) \iff x + 0 = - (-x) \iff x = - (-x)$。
证明：对于实数$x ≠ 0$，有$x × x^{-1} = 1 \iff (x × x^{-1}) ÷ x^{-1} = 1 ÷ x^{-1} \iff x × (x^{-1} ÷ x^{-1}) = (x^{-1})^{-1} \iff x × 1 = (x^{-1})^{-1} \iff x = (x^{-1})^{-1}$。

推论：方程$b = x + a$，对于实数$a$，有唯一解$x = b - a$。
推论：方程$b = x × a$，对于实数$a ≠ 0$，有唯一解$x = b ÷ a$。
证明：对于实数$a$，$b = x + a \iff b - a = (x + a) - a \iff b - a = x + (a - a) \iff b - a = x + 0 \iff x = b - a$。
证明：对于实数$a ≠ 0$，$b = x × a \iff b ÷ a = (x × a) ÷ a \iff b ÷ a = x × (a ÷ a) \iff b ÷ a = x × 1 \iff x = b ÷ a$。

推论：对于实数$x$，有$x × 0 = 0$。
推论：对于实数$x$，有$x × (-1) = (-x)$。
证明：对于实数$x$，有$x × 0 = x × (0 + 0) = x × 0 + x × 0 \iff x × 0 = x × 0 - x × 0 \iff x × 0 = 0$。
证明：对于实数$x$，有$x × (-1) + x = x × (-1) + x × 1 = x × [(-1) + 1] = x × 0 = 0 \iff x × (-1) = (-x)$。

定义：对于实数$x ≥ y$，当且仅当$y ≤ x$。
定义：$x < y$，当且仅当$x ≤ y \land x ≠ y$。
定义：$x > y$，当且仅当$y < x$。
推论：要么$x < y$，要么$x = y$，要么$x > y$。

推论：$(x < 0) \land (y < 0) \implies 0 < x × y$。
证明：

推论：$0 < 1$。
反证：若$1 < 0$，则$(1 < 0) \land (1 < 0) \implies 0 < 1 × 1 \implies 0 < 1$，矛盾。
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
牛顿力学建立在欧几里得几何空间上，广义相对论建立在黎曼几何上。
