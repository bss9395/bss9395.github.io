...
Topic:     顺其自然
Title:     公理化人类社会
Revised:   2019-03-18 00:00:00 +08 @ 中国-广东-湛江 +08
Authors:   璀璨星辰
Resources:
---

***这篇文档写得太乱了，我打算写完后，再重写一遍。***
...
条件概率P(B|A)：已知事件A发生的条件下，事件B发生的概率；
条件概率P(B|\bar{A})：已知事件A不发生的条件下，事件B发生的概率；
后验概率P(A|B)：已知事件B发生的条件下，事件A发生的概率；
...
社会信用评分总是发生在一对实体之间，考虑实体乙对实体甲进行评分。事件A表示实体甲守信用，事件\bar{A}表示实体甲

当实体乙遇见实体甲之时，条件概率P(B|A)即表示在实体甲守信用的条件下，实体乙对于实体甲的守信用的信心。当实体乙遇见实体甲之前，条件概率P(B|\bar{A})即表示



信用评分是介于0与1之间的某个小数，0表示完全不可信，1表示完全可信，0.8表示80%可信且20%不可信。但实际上，在社会中不存在完全不可信的实体，也不存在完全可信的实体。因此0可表示死亡的实体，1可表示完美无瑕的实体。
