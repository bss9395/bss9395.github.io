...
法律规范/法律规则，
1. 具有明确的行为指导性，法律规则对其所覆盖的事实范围和行为事项具有确定的规范性。
2. 在于它具体的适用性。法律规则可以为社会成员遵行，也可以为司法官员所援引，并以其作为准则来裁判案件。
3. 在于它具有最大的确定性，非经立法程序不可变易。
4. 在于它具有明显的可预测性。法律规范的明确性、公开性和确定性，使人们可以据以认识相关行为的法律含义，预测相应行为的法律后果，进而调整自己的法律行为。

法律规范都有自己的适用主体，但一般认为法律是对所有人生效的，所以常常将限定主体忽略掉。
有以下几种：
1. 特定的职务主体，
2. 特定社会身份的主体
3. 特定人身身份的主体
...
就法律条文来说，完整具有四个部分的文字表述的条文是极少的。
主要表现为：
1. 有的法律条文常常将假定条件予以省略。
2. 多数条文将适用主体省略，特定的条文则将适用主体加以明确规定。由于法律多数是针对所有社会成员的，在法律条文的一般表述之中，常常将有关主体的文字省略。但凡是针对特定行为主体或者适用主体的法律规范，一般都会将特定的主体加以明确规定，以防止误解的发生，以保障法律被准确适用。
3. 有的法律条文将行为模式予以省略。
4. 有的法律条文的法律后果部分被规定在其它条文或者文件之中。
5. 有的法律条文中甚至只有一个部分，而将其它两个部分予以省略或者交由另外的法律条文来加以规定。
...
# 法律规范之判例法的逻辑结构
{先前判例}具有代表性的{证据}与{事实}，以及采用{行为模式}与适用{强制措施}；
{当前案件}具有相类似的{证据}与{事实}；
类比推理：{当前案件}类比于{先前判例}，理应采用{行为模式}与适用{强制措施}；
归纳推理：{当前案件}归类于{先前判例}，均应采用{行为模式}与适用{强制措施}；
