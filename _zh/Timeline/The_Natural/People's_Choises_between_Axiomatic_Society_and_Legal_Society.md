...
Revised:   2020-10-21 05:55:00 +08 @ 中国-广东-湛江 +08
Authors:   璀璨星辰
Resources:
---
...
```
# 概率的具体定义
[古典概率] 古典概率 = 事件发生的样本个数 / 事件空间的样本总数； ℙrobability(Event) = Number(Event) / Number(Space)
[频率概率] 频率概率 = 事件发生的试验次数 / 事件空间的试验总数； ℙrobability(Event) = Trial(Event) / Trial(Space)
[几何概率] 几何概率 = 事件发生的区域大小 / 事件空间的区域总体； ℙrobability(Event) = Size(Event) / Size(Space)
[主观概率] 主观概率 = 事件发生在人类的主观判断中的可能性程度；  ℙrobability(Event) ≈ Confidence(Event)

# 概率的典型示例
## 古典概率
随机抛掷一枚均匀硬币，硬币出现正面与出现反面的可能性相当。
事件E为出现正面；事件E发生的样本个数为1；事件空间的样本总数为2；
古典概率ℙ(E) = N(E) / N(𝟙) = 1 / 2

## 频率概率
随机抛掷一枚均匀硬币，统计该硬币出现正面与出现反面的频率。
事件E为出现正面；事件E发生的试验次数为1001；事件空间的试验总数为2000；
频率概率ℙ(E) = T(E) / T(𝟙) = 1001 / 2000

## 几何概率
将一纸带任意截成两段，纸带的左半段与右半段以中点互补对称。
事件E为左半段短于右半段；事件E发生的区域大小为0.5，事件空间的区域总体为1；
几何概率ℙ(E) = S(E) / S(𝟙) = 0.5 / 1

## 主观概率
在街头任意遇见一路人，估计人群中智商的高低近似成自然分布。
