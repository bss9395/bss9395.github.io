...
Revised:   2020-09-27 06:38:00 +08 @ 中国-广东-湛江 +08
Authors:   璀璨星辰
Resources:
---
...
法律规范可划分为法律原则与法律规则。法律原则通常就是共同遵循的基本约定，其适用范围较广，并非针对具体的行为。法律规则以法律条文为典型的表述形式，其适用比较明确，通常针对具体的行为。任何规则的逻辑结构都不超出谓词逻辑的范畴，制定法的逻辑结构都符合必然性谓词逻辑推理，判例法的逻辑结构都符合或然性谓词逻辑推理。

对于制定法的法律条文，
其精细逻辑结构为：对于所有{限定主体}，若满足{触发条件}，则采取{行为模式}，以及适用{强制措施}，{例外情形}特别对待；
其粗放逻辑结构为：对于所有{触发条件含限定主体}，将采用{行为模式含强制措施}，{例外情形}特别对待。

在常见情形下，法律条文几乎都不具备完整的精细逻辑结构，
不仅因为难以准确界定{限定主体}与{触发条件}这两个要件，
而且通常没有必要区分{行为模式}与{强制措施}这两个要件，
但是完整的精细逻辑结构在刑事法中用于定罪量刑却很合适，
此时{限定主体}用于犯罪主体，且{触发条件}用于犯罪程度，
同时{行为模式}用于确定罪名，且{强制措施}用于量度罪刑。


对于判例法的先前判例，
其精细逻辑结构为：{先前判例}具有典型代表性的{案件证据}与{案件事实}，应当采取{行为模式}，以及适用{强制措施}，{附带意见}额外对待；
其租房逻辑结构为：{先前判例}具有典型代表性的{案件事实含案件证据}，应当采用{行为模式含强制措施}，{附带意见}额外对待。
...
1. 判决的必要根据，称为判决理由，
2. 法官们陈述的意见，称为附带意见。
其中只有判决理由可以作为今后遵循的法律规则，附带意见则没有这种权威。
判例法的适用尤其独特的方法，法官在审理案件时，首先必须寻找与当前案件相似的先例，先例中的法律事实必须与当前的案件真实在实质上是类似的，然后他应从先例中提炼出一定的法律原则，再适用于当前的案件。由于与当前案件事实相近的先例有许多，法官为了找出最适合的先例，就必须运用区别技术，对先例中的案件事实与本案事实是否相符作出判断，并对先例中的判决理由和附带意见作出区分，在此基础上选择应适用的判例。

尽管许多普通法系的法官声称确定性是判例法遵循先例原则的优点之一，但实际上一个判例中究竟包含什么样的法律原则，以及它应当适用于什么场合，在很大程度上由法官决定。由于判例法不仅来自法官判决，也由法官来确认和发展他们，因此判例法又被称为法官法。
...
# 法律规范之判例法的逻辑结构
已知前提：{先前判例}具有典型代表性的{案件证据}与{案件事实}，应当采取{行为模式}，以及适用{强制措施}，{附带意见}额外对待。
遵循判例：{当前案件}具有完全相类似的{案件证据}与{案件事实}。
类比推理：{当前案件}类比于{先前判例}，理应当采取{行为模式}，以及适用{强制措施}；
归纳推理：{当前案件}归类于{先前判例}，均应当采取{行为模式}，以及适用{强制措施}。

已知前提：{先前判例}具有典型代表性的{案件证据}与{案件事实}，应当采取{行为模式}，以及适用{强制措施}，{附带意见}额外对待。
创造判例：{当前案件}具有部分相类似的{案件证据}与{案件事实}，但存在明显例外的{案件证据}与{案件事实}；
