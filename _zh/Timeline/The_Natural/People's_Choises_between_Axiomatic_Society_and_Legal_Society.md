...
等额分期存款可通过按期初结算的终值公式来计算每次存款额，若不考虑通货膨胀因素，则终值因子按实际年利率进行逆向复利计算。假设在实际年利率恒为$r = 5\%$的条件下，希望在$n = 3年$后得到存款连本带息的终值为$FV = 10000美元$，则每次存款额为$PMT = \left. FV \middle/ \dfrac{(1 + r)^n - 1}{r} \right. = \left. 10000 \middle/ \dfrac{(1 + 5\%)^3 - 1}{5\%} \right. ≈ 3172.085646美元$，最终累积存款额为$PMT × 3 ≈ 3172.0856 × 3 ≈ 9516.256938美元$。若考虑通货膨胀因素，则每次存款额也即名义每次存款额将随通货膨胀因子而逐步升高，而终值因子按名义年利率进行复利计算。假设在通货膨胀率恒为$π = 3\%$的条件下，名义年利率恒为$i = (1 + r) × (1 + π) - 1 = (1 + 5\%) × (1 + 3\%) = 8.15\%$。
...
等额分期贷款有等额本金与等额本息两种偿还方式，在等额本金偿还方式中，每笔还款额的本金相等，而且在往后每笔还款额中尚未偿还余额的利息逐步降低。在等额本息偿还方式中，每笔还款额的本金与尚未偿还余额的利息之和相等，而且在往后每笔还款额中本金逐步升高，尚未偿还余额的利息逐步降低。等额本息偿还方式可通过按期末结算的现值公式计算每笔还款额，假设在年利率为$6\%$也即每月利率为$i = \dfrac{24\%}{12} = 2\%$的条件下，总贷款额为$PV = 10000美元$，共计分3个月偿还，则每期偿还支付额为$PMT = \left. PV \middle/ \dfrac{1 - (1 + i)^{-n}}{i} \right. = \left. 10000 \middle/ \dfrac{1 - (1 + 2\%)^{-3}}{2\%} \right. ≈ 3467.546726美元$，总还款额为$PMT × 3 = 3467.546726 × 3 = 10402.640178美元$。
