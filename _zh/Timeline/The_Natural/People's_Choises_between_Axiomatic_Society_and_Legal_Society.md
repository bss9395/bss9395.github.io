...
Revised:   2020-10-22 04:14:00 +08 @ 中国-广东-湛江 +08
Authors:   璀璨星辰
Resources:
---
...
在概率论的发展史上，
概率的具体定义曾出现过古典概率/频率概率/几何概率/主观概率，
其中以古典概率为典型，概率论最初便建立在古典概率的基础之上。

古典概率的基本思想有：
事件空间的样本点离散分布；
事件空间的样本点数量有限；
每个事件样本点等可能发生。
因此事件发生的概率等同于事件发生的样本个数与样本总数之比值，
古典概率的计算必须已知事件发生的全部样本，不可凭借趋势推测。

频率概率的基本思想有：
随机试验总是可独立重复进行；
单个随机试验呈现偶然不定性；
大量随机试验呈现统计规律性。
因此事件发生的频率等同于事件发生的试验次数与试验总数之比值，
随着试验重复次数的增加，该频率趋于稳定，此即事件发生的概率。

几何概率的基本思想有：
事件空间的样本点落于区域；
事件空间的总区域范围有限；
等大小事件区域等可能发生。
因此事件发生的概率等同于事件发生的区域大小与区域总体之比值，
单个事件样本点因不可测度大小，其概率没有意义而通常忽略不计。
