...
Revised:   2020-02-16 04:57:00 +08 @ 中国-广东-湛江 +08
Authors:   璀璨星辰
Resources:
---
...
实际上，各国的银行体系总可以看成以中央银行为最后贷款人，除中央银行外其余银行均保留有一定比率的准备金。假设一国银行体系中的法定准备金存款比率为$rd$，中央银行拥有存款$a亿美元$，若中央银行将除准备金之外的存款又作为贷款存入第二银行，则第二银行拥有存款$(1 - 0.1) × a亿美元$，此时经济中的货币供给量为$[1 + (1 - rd)] × a亿美元$。
...
> 第N 银行贷款 &= (1 - rd)^N × a亿美元 \\
> \cdots       &  \cdots \\
> 总货币供应量 &= [1 + (1 - rd)^1 + (1 - rd)^2 + \cdots + (1 - rd)^N + \cdots] × a亿美元 \\
>           &= \dfrac{1}{rd} × a亿美元 \\
> \end{aligned}
> $$
> $$
> \begin{aligned}
> &  等比数列公式： \\
> & 1 + x^1 + x^2 + \cdots + x^N = \dfrac{1 - x^{N + 1}}{1 - x} \\
> & \lim_{N \to ∞} \dfrac{1 - x^{N + 1}}{1 - x} = \dfrac{1}{1 - x} \\
> \end{aligned}
> $$ 
...
杠杆率 利率/复利 分期贷款 存款/股票/债券/保险/期货 金融市场/金融监管 股权/债权/所有权/合同 企业/公司 通货膨胀/政府债务 国内生产总值/物价指数 基尼系数 税收/社会保险/失业率 金融危机
