...
2[正则性] ℙ(E) ≤ 1
3[可加性] ℙ(E ∨ F) ≤ ℙ(E) + ℙ(F)
4[互斥性] [E ∧ F = 𝟘] ↔ [ℙ(E ∨ F) = ℙ(E) + ℙ(F)]
5[排斥性]     [E → F] → [ℙ(F ⊖ E) = ℙ(F) - ℙ(E)]
6[单调性]     [E → F] → [ℙ(E) ≤ ℙ(F)]
7[排斥性] ℙ(F ⊖ E) = ℙ(F) - ℙ(F ∧ E)
根据公理证明：
1由容斥公理 ⇒  ℙ(E ∨ F) = ℙ(E) + ℙ(F) - ℙ(E ∧ F)
 依变元代入 ⇒ ℙ(E ∨ ¬E) = ℙ(E) + ℙ(¬E) - ℙ(E ∧ ¬E)
 由逻辑等价 ⇒      ℙ(𝟙) = ℙ(E) + ℙ(¬E) - ℙ(𝟘)
1由归约公理 ⇒         1 = ℙ(E) + ℙ(¬E)
2由等值演算 ⇒ ℙ(¬E) = 1 - ℙ(E)
 由归约公理 ⇒     0 ≤ 1 - ℙ(E)
2由等值演算 ⇒  ℙ(E) ≤ 1
3由容斥公理 ⇒ ℙ(E ∨ F) = ℙ(E) + ℙ(F) - ℙ(E ∧ F)
 由等值演算 ⇒ ℙ(E ∧ F) = ℙ(E) + ℙ(F) - ℙ(E ∨ F)
 由归约公理 ⇒        0 ≤ ℙ(E) + ℙ(F) - ℙ(E ∨ F)
3由等值演算 ⇒ ℙ(E ∨ F) ≤ ℙ(E) + ℙ(F)
4依附加前提 ⇒   E ∧ F  = 𝟘
 由归约公理 ⇒ ℙ(E ∧ F) = 0
4由容斥公理 ⇒ ℙ(E ∨ F) = ℙ(E) + ℙ(F)
 由容斥公理 ⇒ ℙ(E ∧ F) = 0
 由归约公理 ⇒   E ∧ F  = 𝟘
4根据循环法 ⇒ [E ∧ F = 𝟘] ↔ [ℙ(E ∨ F) = ℙ(E) + ℙ(F)]
5根据包含法 ⇒ [E → F] → [F ↔ F ∨ E]
 根据排除法 ⇒ [E → F] → [F ↔ (F ∧ ¬E) ∨ E]
...
5由等值演算 ⇒ [E → F] → [ℙ(F ∧ ¬E) = ℙ(F) - ℙ(E)]
5根据差分律 ⇒ [E → F] → [ℙ(F ⊖ E) = ℙ(F) - ℙ(E)]
6由归约公理 ⇒ [E → F] → [0 ≤ ℙ(F) - ℙ(E)]
7依附加前提 ⇒ ℙ(F ⊖ E)
 由等值演算 ⇒ ℙ(F ⊖ E) = ℙ(F ∧ ¬E)
 由等值演算 ⇒ ℙ(F ⊖ E) = ℙ(F ∧ (¬F ∨ ¬E))
 由等值演算 ⇒ ℙ(F ⊖ E) = ℙ(F ∧ ¬(F ∧ E))
7根据排斥性 ⇒ ℙ(F ⊖ E) = ℙ(F) - ℙ(F ∧ E)
