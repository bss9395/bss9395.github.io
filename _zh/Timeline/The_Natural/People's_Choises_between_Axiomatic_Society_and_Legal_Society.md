...
典型的比特币转账交易全过程：
1 生成转账交易
  1.1 收款人的钱包取出密钥对，对收款人公钥计算哈希值并生成收款地址（通常呈现为二维码），将收款地址发送给付款人的钱包（通常可扫描二维码）；
  1.2 付款人的钱包取出密钥对，对付款人公钥计算哈希值并生成付款地址，创建比特币转账交易，交易的输入为付款人公钥，交易的输出为收款地址（包含转账金额）与付款地址（包含转账余额=原有金额-转账金额-交易小费），以私钥对交易进行数字签名，将交易向全网广播；
2 验证转账交易
  2.1 区块链完全节点将接收到的交易放入临时交易池，以交易的付款人公钥验证付款人数字签名，若验证无效则拒绝该交易；
  2.2 计算交易的哈希值，若交易的哈希值以及交易已存在于主链/次链/孤儿交易池（相当耗时的操作），则拒绝该交易（为防止双重支付）；
  2.3 若交易的付款地址所对应的收款地址，不存在于主链/次链/孤儿交易池（相当耗时的操作），则将交易放入孤儿交易池（父交易迟到在后）；
  2.4 若交易的原有余额/转账金额/交易小费/转账余额不满足条件，则拒绝该交易（交易的参数无效）；
3 打包转账交易进新区块
  3.1 区块链完全节点生成新区块时，在新区块头填入时间戳，对主链头区块的区块头计算哈希值，在新区块头填入前区块哈希值；
  3.2 新区块中的首个交易为造币交易，交易没有输入只有输出的收款地址（包含挖矿奖励与新区块所有交易小费的总和，初始挖矿奖励为50BTC，每经过2100个区块就折半）；
  3.3 将有效的交易打包进新区块，若孤儿交易池中有子交易且验证通过，则将子交易打包进新区块；
  3.4 对新区块所有交易计算根哈希值，在新区块头填入交易根哈希值，在新区块头不断变更随机数，对新区块头计算哈希值，若小于当前难度系数下的目标值（难度系数由区块链完全节点各自独立调节但全网统一），则将新区块向全网广播（工作量证明机制）；
4 验证新区块
  4.1 其它区块链完全节点将接收到的新区块放入临时区块池，若新区块头的时间戳不满足条件，则拒绝该新区块（防止中间人攻击）；
  4.2 对新区块所有交易计算根哈希值，若验证新区块头根哈希值无效，则拒绝该新区块（防止数据被篡改）；
  4.3 若新区块头前区块哈希值，也即父区块头之哈希值，不存在于主链/次链/孤儿区块池，则将新区块放入孤儿区块池（父区块迟到在后）；
  4.4 验证新区块中所有交易，若存在无效的交易（非常耗时的操作）则拒绝该新区块；
  4.5 将有效的新区块添加到区块链，若孤儿区块池中有子区块且验证通过，则将子区块链接到新区块；
  4.6 剔除临时交易池与孤儿交易池中重复的交易，对比主链与次链切换到长度最长的区块链分支；
  4.7 当有6个子孙区块连续链接到该区块，则该区块便不可逆转被确认有效，此时可修剪已确认无效的次链，被修剪的次链上所有交易全都失效；
5 更新钱包数据
  5.1 收款人的钱包与付款人的钱包按某种规则更新数据，转账交易完成；
```
