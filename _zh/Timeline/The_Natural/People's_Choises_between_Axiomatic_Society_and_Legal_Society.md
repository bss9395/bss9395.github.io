...
金融资产估值通常会估算该资产在未来所产生的现金流在现在的现值，以及在未来全部时间段内的综合收益利率。由于未来的变化总是难以预测，而不同时间点的现金流折算到现在，总是比折算到未来更为准确，因此通过对比现值能够判断金融资产的增值快慢，而通过对比收益利率能够判断金融资产的价值高低。不过，金融资产在未来不同时间段的现金流$CF$一般并不相等，为方便起见，通常总是假定金融资产的综合收益利率$i$按相等时间间隔进行复利计算，若考虑$n年$后还可得到回报终值$FV$，则任何金融资产的价值都可通过金融资产估值公式来计算。

考虑组合投资不同利率的2种零息债券，假设第1种1年到期的债券收益利率为$i_1 = 10\%$面值为$FV = 100美元$，第2种2年到期的债券收益利率为$i_2 = 12\%$面值为$FV = 120美元$，则所有债券的估值为$PV_1 + PV_2 = \dfrac{FV_1}{(1 + r_1)^1} + \dfrac{FV_2}{(1 + r_2)^2} = \dfrac{100}{(1 + 10\%)^1} + \dfrac{200}{(1 + 12\%)^2} ≈ 229.797980美元$，若令$i_1 = i_2$可得综合收益率$i = 17.5534\%$。
...
\begin{aligned}
   &  金融资产估值公式： \\
PV &= \dfrac{CF_1}{(1 + i)^1} + \dfrac{CF_2}{(1 + i)^2} + \cdots + \dfrac{CF_n}{(1 + i)^n} + \dfrac{FV}{(1 + i)^n} \\
...
YTM &≈ \dfrac{PMT + \dfrac{FV - PV}{n}}{\dfrac{FV + 2 · PV}{3}} \\
\end{aligned}
$$
...
对于一个投资者来说，期望现金流是由期望股利和股票出售时的期望价格构成的。

除非一家公司破产清算或者被卖给另一家公司，否则提供给股票持有者的现金流仅由一系列股利构成。每只股票的价值必须等于期望股利的现值，这些股利贯穿公司的整个生命周期。

与债权类似，优先股有面值，优先股股利与利息支付类似，因为它的金额是固定的，而且必须在支付普通股鼓励前支付。

优先股股利是固定费用，会减少分配给普通股股东的金额，因此优先股类似于债务。

投资者通常会预期大多数公司的收益和普通股股利每年都会增加。即使对不同公司的增长预期有所不同，投资者也常常预期股利增长的速度在可以预见的未来保持不变，与名义GNP(这是GNP加通货膨胀率)相同。基于这点，可以预测一家平均或正常的公司的股利每年增长3%-5%。股利增加主要是由于每股收益的增长，导致收益增长的因素有很多，包括通货膨胀、公司的留存收益和再投资收益、权益报酬率。

固定增长率预测值，固定增长率假设对于许多成熟的大公司都是合理的。
