...
Revised:   2020-08-15 18:24:00 +08 @ 中国-广东-湛江 +08
Authors:   璀璨星辰
Resources:
---
...
[公设与公理] 公设即广受认可的假设，公理即理所当然的公设，原理即不证自明的公理；
[推理规则集] 由必然性推理规则得到的结论必然有效，由或然性推理规则得到的结论或然有效；
[推论与定理] 推论即可被推理得到的结论，定理即已被证明的典型推论；
...
从直观上来看，在直线数轴上任意固定一点作为原点，可以将直线数轴对称拆分成方向相反的两条射线，原点即为直线数轴上唯一的中性点。在直线数轴上，通常约定以数字零代表原点，以数字一代表正方向上单位刻度的点。因此，任意一个正整数，均可以表示成从原点出发，以数字一为等距间隔，朝正方向递增该正整数的大小多次所得到的点。相反，任意一个负整数，均可以表示成从原点出发，以数字一为等距间隔，朝负方向递增该负整数的大小多次所得到的点。由此可见，任意一个整数，均可以表示成将该整数所代表的点与原点之间的线段，等距分隔一次所得到的线段。依此类推，任意一个分子为整数且分母为正整数的可通约量，均可以表示成将分子所代表的点与原点之间的线段，等距分隔分母的大小多次所得到的线段。也就是说，任意一个有理数均可以转化为可通约量。从直观上来看，将可通约量的分子与分母同时乘以一个正整数，可以表示分子所代表的线段与分母所代表的线段同时扩大相同倍数，扩大前的可通约量与扩大后的可通约量在直线数轴上始终等比例保持相似性，因此可通约量的值保持不变。对于任意两个可通约量的加法运算，先将分母彼此相乘，并将分子等比例扩大，再将扩大后的分子相加，可以表示两个通约量以相同分母的大小进行等距分隔。对于两个可通约量的乘法运算，先将第一个可通约量的分子乘以第二个可通约量的分子，再将第一个可通约量的分母乘以第二个可通约量的分母，可以表示第一个可通约量先将其分子所代表的线段扩大后，再将其分母所代表的线段等距分隔。对于任意两个可通约量的减法运算，若这两个可通约量相等，则其结果必定为数字零，表示数字零加上任意可通约量的结果为该可通约量。因此数字零对于减法运算具有特殊性，在减法运算中减去一个可通约量，就相当于加上这个可通约量关于数字零反向互补的可通约量。对于任意两个可通约量的除法运算，若这两个可通约量相等，则其结果必定为数字一，表示数字一乘以任意可通约量的结果为该可通约量。因此数字一对于除法运算具有特殊性，在除法运算中除以一个可通约量，就相当于乘以这个可通约量关于数字一反向互补的可通约量。显而易见，直线数轴上任意一个实数，要么为可通约量也即有理数，要么为不可通约量也即无理数。但是以人类现有的认知水平，还无法直接理解无理数的四则运算，因此无理数应先转换成小数形式。有限小数与无限循环小数总是可以表示成可通约量，但无限不循环小数不可以表示成可通约量，因此有限小数与无限循环小数即为有理数，无限不循环小数即为无理数。无限不循环小数可以借助有限小数经无限逼近过程获得任意精度的近似值，这就等价于无理数可以借助有理数经无限逼近过程获得任意精度的近似值，因此理论上无理数的四则运算可以近似借助有理数的四则运算来理解与定义，这正是数学史上建立实数时所采用的古典构造方法。但是任意精度的近似值都必然有误差存在，因而以近似值限定范围的实数，在运算中为保持表示上的一致，就不可避免要对误差进行估算，这会使得看似简单的运算过程变得很繁琐。
...
在实数的古典构造方法中，无理数的精确值由无限不循环小数表示，所有的运算性质仅对有理数有精确定义，因此无理数在运算过程中有别于有理数。现代数学采用公理化的方法建立实数体系，将实数应当遵循的运算性质提炼成为公理，只要实数的所有运算在公理化系统中始终保持自洽，那么实数的具体形式就无关紧要。对于有下确界但无上确界的正整数，仅在直线数轴的正方向上可以找到相对应的点。加法运算应当满足结合律与交换律，以及保持运算的封闭性，这在群论当中被称为可交换的半群。乘法运算应当满足结合律与交换律，以及保持运算的封闭性，此外还应当对加法运算满足分配律。对于任意两个正整数，均可以互相比较大小，而排列在直线数轴上，这在群论当中被称为线性序。满足线性序的正整数，也应当满足小于关系的传递性，以及应当满足小于关系的可加性与可乘性。正整数中的最小值为数字一，而且任意正整数与数字一相加之和必然大于该正整数。采用数学归纳原理，若定义任意正整数有唯一后继，为该正整数与数字一相加之和，则所有的正整数均可以被唯一确定，这正是数学史上建立正整数时所采用的古典构造方法。自然数的运算性质与正整数的运算性质基本类似，不同之处仅仅在于自然数以数字零为最小值。对于无下确界也无上确界的整数，负整数在直线数轴的负方向上可以找到相对应的点，任意正整数与其负数关于原点也即数字零相对称。数字零在减法运算中具有特殊性，据此对于任意整数均可以引入其负元，即为该整数关于数字零相对称的整数。任意整数与数字零相加之和为该整数，任意整数与其负元相加之和为数字零，这在群论当中被称为可交换的群。除此之外，整数的乘法运算为可交换的半群，乘法运算对加法运算满足分配律，这在群论当中被合称为整数环。对于无下确界也无上确界的有理数，负有理数在直线数轴的负方向上可以找到相对应的点，任意有理数与其负数关于原点也即数字零相对称，任意有理数与其倒数关于正方向上的数字一相对称。数字一在除法运算中具有特殊性，据此对于任意有理数均可以引入其逆元，即为该有理数关于数字一相对称的有理数。任意有理数与数字一相乘之积为该有理数，任意有理数与其倒数相乘之积为数字一，这在群论当中被称为可交换的群。有理数连同定义在其上的加法运算与乘法运算，这在群论当中被合称为有理数域。任意两个不同的有理数之间，必定至少存在一个不同的有理数，因此存在无穷多个不同的有理数，有理数在直线数轴上具有无限稠密性。但是任意两个不同的有理数之间，必定至少存在一个不同的无理数，因此存在无穷多个不同的无理数，有理数在直线数轴上具有腔隙性。因此有理数对于自身而言具有连续性，但是有理数本身无法完全与直线数轴上的点建立一一对应关系。虽然以人类现有的认知水平，无法直接理解无理数的运算过程，但是可以理解有理数的运算过程。显然无理数可当作有理数无限逼近的极限值，因此无理数应当具有完全类似于有理数的运算过程。对于无下确界也无上确界的实数，负实数在直线数轴的负方向上可以找到一一对应的点，任意实数与其负数关于原点也即数字零相对称，任意实数与其倒数关于正方向上的数字一相对称。实数连同定义在其上的加法运算与乘法运算，这在群论当中被合称为实数域。由于乘幂对底数为负有理数且指数为整数的运算正负性不连续，因此在实数中仅考虑乘幂对底数为正实数且指数为实数的运算。直观上来看，为解释有理数的四则运算过程，可以参照等距间隔线段的加法运算，为解释底数为正实数且指数为有理数的乘幂，可以参照等距间隔线段的乘法运算。任意一个底数为正实数且指数为正整数的乘幂，可以表示底数自身相乘指数大小多次。那么任意两个底数相同且指数为正整数的乘幂，两者相乘之积为底数不变且指数相加。任意一个底数为正实数且指数为负整数的乘幂，可以表示底数自身相除指数大小多次。任意一个底数为正实数且指数为正整数之倒数的乘幂，自身相乘指数之倒数多次可还原为底数，因此可以表示底数等距分裂正整数多次。那么任意一个底数为正实数且指数为正整数之倒数的乘幂，以其作底数且指数为正整数的乘幂之值，为原有的底数不变且两个指数相乘。任意一个底数为正实数且指数为可通约量的乘幂，可以表示底数自身相乘或者相除指数之分子大小多次，然后等距分裂指数之分母多次。任意一个底数为正实数且指数为无理数的乘幂，应当完全类似于指数为有理数的乘幂，因此任意一个底数为正实数且指数为实数的乘幂，应当满足指数加运算与指数乘运算。
