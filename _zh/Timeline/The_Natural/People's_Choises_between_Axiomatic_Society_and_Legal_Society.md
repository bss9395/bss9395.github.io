...
Revised:   2020-08-15 24:00:00 +08 @ 中国-广东-湛江 +08
Authors:   璀璨星辰
Resources:
---
...
在实数的古典构造方法中，无理数的精确值由无限不循环小数表示，所有的运算性质仅对有理数有精确定义，因此无理数在运算过程中有别于有理数。现代数学采用公理化的方法建立实数体系，将实数应当遵循的运算性质提炼成为公理，只要实数的所有运算在公理化系统中始终保持自洽，那么实数的具体形式就无关紧要。对于有下确界但无上确界的正整数，仅在直线数轴的正方向上可以找到相对应的点。加法运算应当满足结合律与交换律，以及保持运算的封闭性，这在群论当中被称为可交换的半群。乘法运算应当满足结合律与交换律，以及保持运算的封闭性，此外还应当对加法运算满足分配律。对于任意两个正整数，均可以互相比较大小，而排列在直线数轴上，这在群论当中被称为线性序。满足线性序的正整数，也应当满足小于关系的传递性，以及应当满足小于关系的可加性与可乘性。正整数中的最小值为数字一，而且任意正整数与数字一相加之和必然大于该正整数。采用数学归纳原理，若定义任意正整数有唯一后继，为该正整数与数字一相加之和，则所有的正整数均可以被唯一确定，这正是数学史上建立正整数时所采用的古典构造方法。自然数的运算性质与正整数的运算性质完全类似，唯一不同之处仅仅在于自然数以数字零为最小值。对于无下确界也无上确界的整数，负整数在直线数轴的负方向上可以找到相对应的点，任意正整数与其负数关于原点也即数字零相对称。数字零在减法运算中具有特殊性，据此对于任意整数均可以引入其负元，即为该整数关于数字零相对称的整数。任意整数与数字零相加之和为该整数，任意整数与其负元相加之和为数字零，这在群论当中被称为可交换的群。除此之外，整数的乘法运算为可交换的半群，乘法运算对加法运算满足分配律，这在群论当中被合称为整数环。对于无下确界也无上确界的有理数，负有理数在直线数轴的负方向上可以找到相对应的点，任意有理数与其负数关于原点也即数字零相对称，任意有理数与其倒数关于正方向上的数字一相对称。数字一在除法运算中具有特殊性，据此对于任意有理数均可以引入其逆元，即为该有理数关于数字一相对称的有理数。任意有理数与数字一相乘之积为该有理数，任意有理数与其倒数相乘之积为数字一，这在群论当中被称为可交换的群。有理数连同定义在其上的加法运算与乘法运算，这在群论当中被合称为有理数域。任意两个不同的有理数之间，必定至少存在一个不同的有理数，因此存在无穷多个不同的有理数，有理数在直线数轴上具有无限稠密性。但是任意两个不同的无理数之间，必定至少存在一个不同的无理数，因此存在无穷多个不同的无理数，有理数在直线数轴上具有腔隙性。因此有理数对于自身而言具有连续性，但是有理数本身无法完全与直线数轴上的点建立一一对应关系。对于无下确界也无上确界的实数，负实数在直线数轴的负方向上可以找到相对应的点，任意实数与其负数关于原点也即数字零相对称，任意实数与其倒数关于正方向上的数字一相对称。实数连同定义在其上的加法运算与乘法运算，这在群论当中被合称为实数域。

虽然以人类现有的认知水平，无法直接理解无理数的运算过程，但是可以理解有理数的运算过程。
据此可以定义乘幂对底数为正实数且指数为有理数的运算。
...
于2020年08月15日 00:24:00 +08 在湛江 +08
