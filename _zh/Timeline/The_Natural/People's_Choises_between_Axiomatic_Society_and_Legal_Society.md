...
# 自然语言中典型的逻辑悖论
(人)会给(人们)立法 = P(人, 人们)
自然语言：{有一些}[(人)会给{所有的}[不为(自己)立法的(人们)立法]]。
...
从给定逻辑值为真的已知前提出发，在逻辑推理的任何步骤，可认为经逻辑推理所得到的推论总是合乎逻辑的，即认为已得推论总是有效的，并且其逻辑值总是为真。而对于下一阶段的逻辑推理，已得推论又可作为给定逻辑值为真的已知前提，再动态加入到已知前提集中。若复合命题的逻辑值永为真，则任何命题变元都可代入任意的命题变项，无论其逻辑值如何也不会影响复合命题的逻辑值。





同时由于推理定律总是有效的，因此在命题逻辑推理的任何步骤，任何命题变项都可以等价的命题变项替换。命题逻辑的推理规则同样适用于谓词逻辑，此时带有量词的谓词与不带量词的谓词均可当作命题。在谓词逻辑推理的任何步骤，任何谓词的任意个体变元都可以其辖域中的常量赋值，此时原本带有量词的谓词就退化成命题。

值得一提的是，在自然语言中始终存在逻辑悖论现象，
