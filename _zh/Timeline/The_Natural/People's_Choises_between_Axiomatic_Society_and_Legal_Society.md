...
# 计数原理
[加法原理] 完成事件有k类途径，完成第k类途径有nₖ种方法，完成事件总共有N种方法。
N = n₁ + n₂ + ⋯⋯ + nₖ

[乘法原理] 完成事件有k个步骤，完成第k个步骤有nₖ种方法，完成事件总共有N种方法。
N = n₁ × n₂ × ⋯⋯ × nₖ

[抽屉原理] 将n个不同元素放入k个抽屉中，次序无先后，存在一个抽屉至少有⌈n / k⌉也即⌊(n - 1) / k⌋ + 1个元素。
⌈n / k⌉ ≡ ⌊(n - 1) / k⌋ + 1。

[有序排列] 从n个不同元素中任取k个元素排成一列，次序有先后，取出又放回0次，总共有Permutation(n, k, 0)种排列。
Permutation(n, k, 0) = n × (n - 1) × ⋯⋯ × (n - k + 1) = n! / (n - k)!

[重复排列] 从n个不同元素中任取k个元素排成一列，次序有先后，取出又放回k次，总共有Permutation(n, k, k)种排列。
Permutation(n, k, k) = n × n × ⋯⋯ × n = nᵏ

[无序组合] 从n个不同元素中任取k个元素排成一列，次序无先后，取出又放回0次，总共有Combination(n, k, 0)种组合。
Combination(n, k, 0) × k! = Permutation(n, k, 0)
Combination(n, k, 0) = n! / (n - k )! / k!

[重复组合] 从n个不同元素中任取k个元素排成一列，次序无先后，取出又放回k次，总共有Combination(n, k, k)种组合。
Combination(n, k, k) = Combination(n + k - 1, k, 0)
Combination(n, k, k) = (n + k - 1)! / (n - 1)! / k!
```

于2020年08月19日 16:41:00 +08 在湛江 +08
