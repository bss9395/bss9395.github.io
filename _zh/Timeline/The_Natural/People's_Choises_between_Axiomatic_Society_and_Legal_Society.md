...
Revised:   2020-04-20 04:27:00 +08 @ 中国-广东-湛江 +08
Authors:   璀璨星辰
Resources:
---
...
![max-width:360px;](figures/Economics_Gini_Coefficient.svg)
...
考虑A国和B国间加上一堵完全隔离的防火墙，那么A国境内的区块链与B国境内的区块链将往不同的方向生长，经过很长一段时间后，再将这堵防火墙拆除，这时候A国境内的区块链与B国境内的区块链将出现硬分叉，两种区块链的数据不一致，也无法融合。


比特币的存储方式应该称作【准分布式存储】，因为每个节点存储的区块链只有一条，而且都一样。如果全网只剩下一个节点，这个数据依然是完整的。
【完全分布式存储】应该是这样，每个节点存储的区块链有多条，不都一样。如果全网只剩下一个节点，这个数据很可能是不完整的。
...
节点，《链接未来》34页
参与记账的每个计算机称为节点。
1. 在区块链系统中，每个节点（通常指计算机）都有一个完整的账本副本，因此所有交易数据系统中的人都可以看到。
2. 每个节点的权利是一样的，任意节点被摧毁都不会影响整个系统的安全，也不会造成数据丢失。其在整个系统中的权重都是一样的。系统每次都在链入这个系统的节点中选择记账者，即使某个或者部分节点宕机，也不会影响整个系统的运作。
3. 篡改单个节点的数据没有意义，因为如果系统发现两个账本对不上，就认为拥有相同账本数量相对较多的节点才是真实的数据版本。那些少部分不一致的节点账本不是真实的，而是被篡改的账本，系统会自动舍弃这部分认为被篡改过的账本，除非能够控制整个系统中的大部分节点，也就是通常所说的51%攻击，才能发动对账本数据的更改。



时间戳
过去，时间戳包括三个部分，文件的摘要/第三方机构收到文件的日期和时间/第三方机构的数字签名。
时间戳产生的过程为，用户首先将需要加时间戳的文件用哈希编码加密形成摘要，然后将该摘要发送到第三方认证机构，该机构在加入收到文件摘要的日期和时间信息后再对该文件加密（数字签名），然后送回用户。


比特币的五个重要特性：
中本聪在《比特币：一种点对点的电子现金系统》中，系统提出去中心化电子货币的理论。
1. 可以用点对点网络解决双重支付问题；
2. 没有类似铸币厂之类的第三方信任机构；
3. 使用者可以完全匿名；
4. 可以用哈希现金形式的工作量证明来制造新的货币；
5. 用于制造新货币的工作量证明机制同样可以用来预防双重支付；


去中心化的共识难题
区块链系统会选取记账最快/最好的那个节点，以这个节点的账目为准，并发送备份给所有节点。
在一个中心化的系统里，因为有权威化的中心存在，要达成共识很容易。
去中心化的记账是一个公开透明的系统，完全不需要第三方监督或者证明：
1. 由于是点对点的沟通，系统中有那么多人，它们都能接收到A的信息，但是所有节点之间却并不认识，也无法交流，如何能够保证所有人的记账都是同步的？
2. 假如有人没有听清楚，把帐记错了，或者包庇A，故意做假账，怎么办？
去中心化系统所面临的两大难题：同步性问题和一致性问题。
在点对点通信，双方不可能在这种情况下达到信息的一致性，就是在分布式计算机上，试图在异步系统和不可靠的通道上达到一致性是不可能的。


共识机制：工作量证明
监测工作的整个过程通常是极为低效的，而监测结果则要高效得多。
工作量证明系统（协议/函数），是一种应对拒绝服务攻击和其它服务滥用的对策，要求发起者进行一定量的运算，也就意味着需要消耗计算机一定的时间；
重要特征是客户端需要做一定难度的工作得出一个结果，验证方却很容易通过结果来检查出客户端是不是做了相应的工作。
区块链让所有节点参与记账，但记账是需要成本的，节点凭什么要这么做？没有奖励，节点就没有参与的动力。
1. 节点处理事务的能力不同，网络节点数据的吞吐量有差异；
2. 节点间通信的信道可能不安全；
3. 可能有作恶节点出现；
在中本聪的设计里，每轮竞争胜出并完成记账的节点将获得系统给予的一定数量的比特币奖励，这个奖励的过程也是比特币的发行过程。
系统发放的奖励包含两部分，一部分是区块所包含交易的手续费，这部分不属于比特币的发行过程，另一个部分是新币奖励，每四年减半，这是比特币的发行过程。
为获得系统发放的比特币，节点不停地进行计算和竞争，同时不断地有新区块产生，这个过程很像现实生活中挖矿的过程，因此获得比特币的过程被人们形象地称为挖矿。


默克尔树
叶子是数据块（文件或者文件的集合）的哈希值，非叶子节点是其对应子结点串联字符串的哈希值。
在P2P网络中，默克尔树用来确保从其它节点接收的数据块没有损坏且没有被替换，甚至检查其它节点不会欺骗或者发布虚假的块使用，在区块链网络中，默克尔树可以快速校验大规模数据的完整性。归纳一个区块中的所有交易信息，最终生成这个区块所有交易信息的一个统一的哈希值，区块中任何一笔交易信息的改变都会使得默克尔树发生改变。
特点如下：
1. 一种树状结构，可以是二叉树，也可以是多叉树；
2. 默克尔树的叶子节点上的值看设计，比如将哈希值作为叶子节点的值。
3. 非叶子节点的值是根据下面所有的叶子节点值，按照一定的算法计算而得出的，比如哈希运算。
默克尔根哈希值存在于每一个区块的头部，通过根哈希值连接着区块体，而区块体内包含着大量的交易，每个交易本身都有自己的哈希值来唯一标识自己。


分叉
比特币是一个大量网络节点共同维护的分布式账本，节点维护账本使用的统一的比特币软件。如果是去中心化的系统，在进行软件升级时只需要在中心服务器上安装升级后的新软件即可，但在分布式系统中进行软件升级就比较麻烦，需要在约定的时间将所有网络节点同时升级软件，如果中间有的节点没有及时升级，就有可能出现分叉。
软分叉：
1. 新版本的软件抢到记账权，打包成区块以后广播出去，老版本的软件验证通过；
2. 老版本软件抢到记账权，打包成区块后广播出去，被所有新版本软件认为区块不合法。
只要有超过50%的节点升级到新版本，就不会出现软分叉，只会造成运行老版本的节点，拿不到奖励；
硬分叉：
如果新版本的软件抢到记账权，打包成区块后广播出去，被老版本认为不合法。
...
比特币的交易全过程
客户将比特币打到商家的地址上，商家在收到客户的比特币后再将商品发出。
1. 商家打开自己的比特币钱包，创建一个新的比特币地址，即创建密钥对，有一个公钥和一个私钥，私钥保存在钱包，公钥形成二维码。二维码发送给客户。
2. 客户使用已有的比特币地址，客户的钱包客户端用自己的私钥对本次交易进行签名，交易的输入地址是客户的公钥的哈希值，交易的输出地址是二维码。客户的钱包客户端向全网广播这笔交易。
3. 全网所有节点或者矿工都会验证这个交易是否有效，验证方法就是从区块链中找出客户的地址对照即可。矿工经过验证确认交易有效后，将这个交易放进内存，等待打包进入数据块。挖矿工作量证明，算出符合条件的随机值，生成合格的区块，并向全网广播这个区块，其它节点收到广播后将数据块添加到区块链中，得到初步确认，当下一个区块链接到此区块后，交易得到进一步确认。在得到6个区块的确认之后，这笔交易就不可逆转被确认。
4. 商家更新客户端数据，发现10个比特币已经到达商家的地址，经过一段时间的等待确认，将商品发送给客户，交易完成。


双重支付问题，有时间差
一种情形，如果客户给商家支付10个比特币的同时，未等系统确认，又用同一个地址（内有11个比特币）向其它地址支付10个比特币，并且采取一些技术手段，比如将10个比特币分散成极小份发送到若干地址里，使得系统优先确认该项交易，使得后一笔交易优先于前一笔得到确认，那么店主最终将得不到那10个比特币。
比特币不适合时效性较强的交易。


钱包的安全问题
私钥存储在钱包上，
1. 若误把钱包文件删除，那么与钱包关联的私钥/公钥/地址，与地址关联的比特币都会丢失。
2. 没有正确备份钱包文件，导致一段时间内交易的比特币全部丢失；
3. 电脑中病毒，钱包文件被盗，所有比特币荡然无存。
钱包的本质是私钥池，存放在这个钱包中所有地址的私钥，在交易时并没有真正产生一个新地址，而只是从私钥池里取出一个使用，同时再生成一个新的私钥和地址并放进私钥池里，使未使用的私钥保持一定数量。存放在wallet.dat文件中，钱包风险：
1. wallet.dat文件被黑客获取，那么黑客同样对钱包里的地址有支配权；
2. wallet.dat文件丢失或误删，则用户所有的比特币都会丢失。那么与这些地址关联的比特币将不属于任何人，因为唯一证明所有权的私钥已经永远消失，但不会使比特币总量减少。
3. wallet.dat备份出错。


客户端本身的找零机制
依照比特币本身的规则，一个地址上的比特币在支付的时候必须全部支出，除向另一个地址支付比特币以外，剩余金额再重新支付给原地址。
比如，A的地址里之前有0.18个比特币，当A的地址拥有者向B的地址支付0.1个比特币以后，系统会将A的地址中0.18个比特币全部取出，并将其中0.1个比特币转入B的地址里，剩余的0.08个比特币重新打回A的地址。
客户端出于保护用户匿名性的考虑，并不会将余额打回原地址，而是从私钥池里再取出一个新地址并将余额打进去。所以每一次交易，私钥池里的地址就被取出一个，同时又有新的私钥补充进去。如果用户对wallet.dat进行备份之后，完成100次交易，那么私钥池里的原有全部私钥就用完了，接下来使用的都是未曾备份的新私钥，使用新私钥完成交易之后，如果用户用原来的备份文件进行恢复，那么所有未备份的新私钥都会丢失，这些新地址里的比特币也会随之丢失。


银行和网站有专门的系统来保护用户资料，不允许黑客多次尝试密码，而比特币的所有地址都在网上公开可查，黑客可以不断试错，直至找到对应的私钥。
从理论上来说，8位数以上的包含95种字符的密码应该是安全的，因为按照目前的计算机的速度，暴力穷举法对8未以上的密码基本上无能为力。

比特币的问题
1. 手续费
2. 转账速度
3. 流通量不够，通货紧缩（但可以拆分，只要发明更小的单位即可，数字货币不像物理货币）
...
《破冰区块链》
比特币转账
例子：小王给小李通过银行卡转账1000元，这个时候银行会在小王的银行账号上进行1000元的减法，在小李的银行账号上进行1000元的加法，以完成这笔交易。
整体过程是：小王会先让小李把银行卡号发给他，小王登录银行网银，找到转账菜单，输入小李的银行卡号，然后再输入金额1000元，屏幕上可能会显示若干元的转账手续费，这时候小王点击确定，然后提交给银行后台去处理。银行处理完毕，小王的账户会减少1000元，小李的银行卡账户里会增加1000元。

比特币的交易
转账发起人向区块链网络发送交易请求；
1. 登录比特币钱包，比特币钱包类似于银行卡的网银；
2. 选择转出方的比特币钱包地址。钱包地址相当于银行卡号，选择转出方的比特币钱包地址就是选择从自己哪个比特币地址进行转账，类似于选择自己转出的银行卡账号。钱包地址就相当于实体钱包里的一张张银行卡。
3. 输入对方的比特币钱包地址，对方的钱包地址相当于对方的银行卡号；
4. 输入转账数额和手续费后提交。输入转账的数额以及转账的交易手续费金额，签上比特币签名，提交给比特币网络，然后就等待这笔交易被打包处理。

参与到比特币网络上的矿工对交易的有效性进行验证，如果验证通过，这笔交易将和其它验证通过的交易一起被共同打包到一个区块中。
转账的手续费不是固定不变的，而是用户可以自行确定转账手续费是多少，甚至可以选择不给转账手续费。如果不给手续费，可能不会被矿工记账确认，或者比较晚或延迟被矿工确认。

矿工基于工作量证明机制，利用算力竞相解答数学难题。最先解答出数学难题答案的矿工，如果其工作量被其它矿工确认，则该矿工将获得记账权，同时获得该新生区块所对应的比特币奖励。
当一个用户给比特币网络提交转账的请求后，需要等着矿工来打包处理。比特币网络规定，竞相计算能获得新生的比特币奖励，谁通过竞争记账获得记账权，谁就可以获得该交易的手续费。
比特币网络每10分钟产生一个区块，通过这种方式，比特币解决在一个去中心化的体系下账本的唯一性的权益。
算得最快的计算机就会挖到一定数量的比特币，在每轮竞争和被记录在账单里的交易手续费。矿工有两个收入来源：一个是新生区块奖励；另一个是区块被记录在区块里面的所有交易的交易手续费。

新生的区块被确认后，成功连接到区块链网络中，而其它比特币节点对其区块上的交易信息进行同步；

转账的收款方最终收到转账发起方的1个比特币。

当由于新区块生成或者网络传播延迟等原因造成区块链产生分歧的状况时，最长链机制，网络上多数节点合意的结果选择最长的那条区块链。

若想把某个区块高度上的某个交易信息进行修改，需要从这个区块开始把之前所有的区块都重新计算一遍，再把账本同步给其他人，而在进行计算的同时，其它矿工们已经在原来的链上继续往前进行计算了，这个恶意篡改的人，需要在很短时间内赶上现有区块的高度，让自己的这条区块链成为最长的链，让其它矿工误以为这是正确的账本。由于最长链机制的存在，除非这个恶意篡改的节点拥有非常大的算力，至少超过50%，否则基本上没有可能进行这样的篡改。



UTXO未花费的交易输出，Unspent Transaction Outputs结构
UTXO不是像传统的银行账户那样去管理和记录账户余额，而是基于交易数据来对未花费的余额进行管理。
比特币区块链上记录的交易，是由输入和输出构成的。某个交易中消费的比特币数量，可以理解为这个交易的输入，而这个交易通过交易流转下去的比特币数量，可以理解为这个交易的输出。在不考虑开工费的情况下，一个交易的输入总额和输出总额应该是一致的。
比特币就在一连串的交易中流转，某个交易中被流转出来的比特币可能谁也没有消费，就成该交易中未花费的交易输出，而这笔交易已经花费的输出就成了交易的接收方的输入。交易构成一组链式结构，所有合法的比特币交易都可以追溯到前向一个或多个交易输出。通过UTXO的方式，传递每一笔交易的输入和输出，交易数据就在区块链上得以维护。同时一个用户的余额因此并不是作为一个数字存储起来的，而是用它占有的UTXO的综合计算出来的。

挖矿交易，通过UTXO结构，在比特币世界里每一笔交易都能追溯到上一笔交易，每一笔付款都可以追溯到上一笔收款。除非直接挖矿所得，否则必须是从上一个交易得到的并且还没有花费的比特币。一笔一笔往上追溯，可以追溯到该比特币诞生时矿工挖出来的那个区块。比特币被矿工挖出来的那一笔交易是作为挖矿奖励发给矿工的，是比特币的源头交易，只记录输出，没有输入信息，这种特别的交易叫做挖矿交易。

假设A拥有5UTXO/15UTXO，其中3UTXO转账给B，则5UTXO作为交易的输入，3UTXO与B的地址关联，剩下的2UTXO与A原有的地址关联。

UTXO结构遵守的两个规则：
1. 交易有源头，除挖矿交易外，所有的交易都必须来自前面某一个或几个交易的UTXO，该交易必须包含一个有效的签名，来自他所花费的UTXO的拥有者；
2. 输入输出持平，任何一笔交易的输入总量必须等于输出总量。
...
1. 叶节点存储的是数据文件；
2. 非叶节点存储的是其子节点的哈希值；
3. 底层数据的任何变动，都会传递到其父节点，一直到树根；
这些非叶子节点的哈希值被称作路径哈希值，可以根据其确定某个叶子节点到根结点的路径，叶结点的哈希值是真实数据的哈希值。
...
目前的区块链系统，尤其是金融区块链系统中，存在数据确认时间较长的问题。当前比特币交易的一次确认时间大约需要10分钟，6次确认的情况下，需要等待约1小时。

3. 处理交易频率过低

区块链系统面临交易频率过低的问题，每条交易的平均大小约250个字节，如果区块大小限制在1MB，那么可以容纳的交易数量为4000条。按照每10分钟产生一个区块的速度计算，每天可以产生144个区块，也就是能容纳576000条交易，在除以每天的秒数86400，比特币区块链最高每秒处理6.67笔交易。比特币区块链上每天的实际交易量已经接近系统瓶颈，如果扩容问题得不到解决，可能造成大量交易的阻塞延迟。

相比之下，PayPal在2013年第三季度的总体交易笔数为7.29亿笔，平均每秒为93.75笔交易，全球最大的支付卡VISA在2013年的测试中实现每秒处理47000笔交易。




简单支付验证SPV：无须维护完整的区块链信息，只需要保存所有的区块头部信息即可进行支付验证的技术。节点虽然不能独立验证交易，但能够从区块链其它节点获取交易验证的必要信息，从而完成交易支付验证，同时还可以得到整个区块链网络对交易的确认数。

SPV节点与轻钱包的区别：
轻钱包指的是节点本地只保存与其自身相关的交易数据，尤其是可支配交易数据，但并不保存完整区块链信息的技术。
SPV的目标是验证某个支付是否真实存在，并得到多少个确认。
如何快速验证某笔交易支付的真实性，是SPV的工作目标。轻钱包的目标不仅是支付验证，而且是用于管理节点自身的资产收入、支付等信息。在本地只保存与自身相关的交易数据，尤其是可支配的交易数据。
轻钱包仍然需要下载每个新区块的全部数据并进行解析，获取并本地存储与自身相关的交易数据，只是无须在本地保存全部数据而已。而SPV节点不需要下载新区块的全部数据，只需要保存区块头部信息即可。

支付验证与交易验证的区别：
SPV指的是支付验证，而不是交易验证。
区块链交易验证的过程比较复杂，包括账户余额验证/双重支付判断等等，通常由保存区块链完整信息的区块链验证节点来完成。
支付验证过程比较简单，只是判断该笔支付交易是否已经得到区块链节点共识验证，并得到多少确认数即可。
交易验证的过程如下，完全节点A遍历完整的区块链账本，在区块链账本的交易中保存鲍伯的历史交易信息，包括鲍伯的汇款账户/鲍伯的签名/历史收款人的地址/汇款金额信息等等，查询鲍伯的账户就可以判断鲍伯提供的账户是否有足够的余额，如果余额不足则交易验证失败。其次，完全节点要根据区块链账本判断鲍伯是否已经支出了这个账户上的钱给别人，即是否存在双重支付问题，如果存在则交易验证失败；最后，判断鲍伯是否拥有其提供账户的支配权，如果判断失败则交易验证失败。

SPV验证的工作原理：
1. 计算待验证支付的交易哈希值；
2. 节点从区块链网络上获取并存储最长链的所有区块头至本地；
3. 节点从区块链获取待验证支付对应的默克尔树哈希认证路径；
4. 根据哈希认证路径，计算默克尔树的根哈希值，将计算结果与本地区块头中的默克尔树的根哈希值进行比较，定位到包含待验证支付的区块；
5. 验证该区块的区块头是否已经包含在已知最长链中，如果包含则证明支付有效；
6. 根据该区块头所处的位置，确定该支付已经得到确认的数量；

最复杂的步骤是3，节点从区块链获取验证支付对应的默克尔树哈希认证路径的过程，需要建立布鲁姆过滤器。


区块链的工作流程：
1. 发送节点将新的数据记录向全网进行广播；
2. 接收节点对收到的数据记录信息进行检验，比如记录信息是否合法，通过检验后，数据记录将被纳入一个区块中；
3. 全网所有接收节点对区块执行共识算法（工作量证明）
4. 区块通过共识算法过程后被正式纳入区块链中存储，全网节点均表示接收该区块，而表示接收的方法，就是将该区块的随机散列值视为最新的区块散列值，新区块的制造将以该区块链为基础进行延长。

节点始终都将最长的区块链视为正确的链，并持续以此为基础验证和延长他。
