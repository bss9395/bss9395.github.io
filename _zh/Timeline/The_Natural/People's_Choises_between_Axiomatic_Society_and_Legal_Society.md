...
Revised:   2020-08-16 03:26:00 +08 @ 中国-广东-湛江 +08
Authors:   璀璨星辰
Resources:
---
...
[推论与定理] 推论即可被推理得到的结论，引理即辅助推理证明的推论，定理即已被证明的典型推论；
...
在实数公理系统中，由全部公理组成的公理集相当于最小完备的内核，实数的所有运算性质都可以根据公理集推导出，但是仅仅具有公理集显然还不足以建立实数。



也就是说除公理集以外，定义集与推理规则集几乎具有同等重要的作用。
以人类的认知水平，可以直接理解正整数的加法运算与乘法运算，这在公理集中被提炼成为加运算公理与乘运算公理，减法运算与除法运算同为最常用的四则运算，实际上也可以被提炼成为减运算公理与除运算公理。
但是由于加法运算与减法运算以及乘法运算与除法运算的运算方向互为相反，因此这两组公理之间存在不必要的冗余，并不满足公理系统的典型性质中的弱独立性。
通常来说，在公理系统中只考虑最为基本的公理，如果两组公理彼此相关联，那么相对较复杂的公理就可以当作定义，并由更为基本的公理经过额外的步骤推导出。
除此以外，那些影响公理系统具体形态特征的性质，也可以由定义而不是公理给出，比如根据人类的思维习惯，四则运算应当采用左结合性，以及乘运算优先于加运算。显然这些定义并非唯一确定，但是不同的定义也不会改变公理系统的基本形态特征，为简化运算过程应当尽可能采用良好的定义。


通常来说，公理系统建立在一些广受认可的假设之上，这些假设相对于公理系统即为公设，除此以外，还有必要借助于一些几乎无从证明的规律，这些规律相对于公理系统即为原理，所有的公设与原理均符合人类的经验常识与思维习惯，因此原则上可认为其本身不证自明。

通常来说，公理系统中的演绎规则等同于逻辑推理中的必然性逻辑推理，但并不涉及或然性逻辑推理，也即等同于逻辑推理中的推理规则与推理定律。这是因为由必然性逻辑推理得到的结论必然有效，由或然性逻辑推理得到的结论或然有效，而在公理系统中仅考虑确切无误的逻辑推理。

值得一提的是，由于社会科学的思维严谨性不如自然科学，也难以将自然语言以符号化形式转换成逻辑语言，因此对于社会科学语境下的公理系统，通常不得不掺杂或然性逻辑推理。


类似于逻辑推理系统，在公理系统中由公设与公理以及定义等共同组成已知前提集，由原理与定律以及演绎规则等等共同组成推理规则集。

通常将相当常用或者相当典型的推论称为定理，
而将辅助定理证明的常用推论称为引理。
在公理系统中，定理几乎处于与公理同等重要的地位，将定理作为已知前提，显然可以加快公理系统的逻辑推理进程。
