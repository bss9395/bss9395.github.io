...
Revised:   2020-02-24 02:02:00 +08 @ 中国-广东-湛江 +08
Authors:   璀璨星辰
Resources:
---
...
金融资产估值通常会估算该资产在未来所产生的现金流在现在的现值，以及在未来全部时间段内的综合收益利率。由于未来的变化总是难以预测，但将不同时间点的现金流折算到现在，总是比折算到未来更为准确，因此通过对比现值能够判断金融资产的价值高低，而通过对比收益利率能够判断金融资产的增值快慢。不过，金融资产在未来不同时间段的现金流$CF$一般并不相等，为方便起见，通常总是假定金融资产的综合收益利率$i$按相等时间间隔进行复利计算，若考虑$n年$后还可得到回报终值$FV$，则任何金融资产的价值都可通过金融资产估值公式来计算。考虑组合投资不同具有收益利率的零息债券，零息债券以低于票面面值的发行价发行，不附加息票且只在到期日按面值进行一次性兑付，因此零息债券的发行价也即估值应定为$PV_t = \dfrac{FV_t}{(1 + i_t)^t}$。假设购买1年到期的零息债券1张，票面面值为$FV_1 = 100美元$票面收益利率为$5.2632\%$，购买2年到期的零息债券2张，票面面值为$FV_2 = 120美元$票面收益利率为$i_2 = 5.9007\%$，5年到期的零息债券5张，票面面值为$FV_3 = 500美元$票面收益利率为$i_3 = 7.5173\%$。则5年到期零息债券的发行价为$PV_3 = \dfrac{500}{(1 + 7.5173\%)^5} = 348美元$，购买全部零息债券的价值总计为$PV = \dfrac{FV_1 × 1}{(1 + i_1)^1} + \dfrac{FV_2 × 2}{(1 + i_2)^2} + \dfrac{FV_3 × 5}{(1 + i_3)^5}$ = 95 × 1 + 107 × 2 + 348 × 5 = 2049美元，若令$i_1 = i_2 = i_3$可得这项组合投资的综合收益率为$i ≈ 7.4197\%$。考虑投资具有固定利率的付息债券，付息债券按期支付利息且在到期时按面值进行兑付，因此付息债券的发行价也即估值为$PV = \sum\limits_{t = 1}^n \dfrac{PMT}{(1 + i)^t} + \dfrac{FV}{(1 + i)^t}$。假设10年到期按年支付利息的付息债券，票面面值为$FV = 1000美元$票面收益利率为$i = 5\%$，则该付息债券每年支付利息$PMT = FV × i = 1000 × 5\% = 50美元$，发行价为$PV = 50 × \dfrac{1}{5\%} \left[ 1 - \dfrac{1}{(1 + 5\%)^{10}} \right] + \dfrac{1000}{(1 + 5\%)^{10}} = 1000美元$，发行价与票面面值相等。若该付息债券在发行时，市场收益利率变动为$i = 6\%$，则该付息债券的市场价格也即估值为$PV_0 = 50 × \dfrac{1}{6\%} \left[ 1 - \dfrac{1}{(1 + 6\%)^{10}} \right] + \dfrac{1000}{(1 + 6\%)^{10}} ≈ 926.399129美元$。因此，当市场收益利率高于票面收益利率时，市场价格低于票面面值，此时应按市场价格发行该付息债券，也即折价发行。若该付息债券发行4年后，市场收益利率变动为$i_4 = 4\%$，则该付息债券的市场价格也即估值为$PV_4 = 50 × \dfrac{1}{4\%} \left[ 1 - \dfrac{1}{(1 + 4\%)^{10 - 4}} \right] + \dfrac{1000}{(1 + 4\%)^{10 - 4}} ≈ 1052.421369美元$。因此，当市场收益利率低于票面收益利率时，市场价格高于票面面值，此时若按市场价格转手该付息债券，则显然会获利。此时投资者若按市场价格卖出该付息债券，则4年内该付息债券的综合收益利率由金融资产估值公式$PV_0 = \sum\limits_{t = 1}^4 \dfrac{PMT}{(1 + i_{04})^t} + \dfrac{PV_4}{(1 + i_{04})^4}$计算为$i_{04} ≈ 8.398415\%$。若该付息债券按市场收益利率恒为$i = 4\%$，则该付息债券发行5年后，市场价格也即估值为$PV_5 = 50 × \dfrac{1}{4\%} \left[ 1 - \dfrac{1}{(1 + 4\%)^{10 - 5}} \right] + \dfrac{1000}{(1 + 4\%)^{10 - 5}} ≈ 1044.518223美元$。从直观上来看，当$i_0 > i_4$时有$PV_4 > PV_5$，且随着时间越来越趋近到期日，该付息债券的市场价格将越来越趋近票面面值，也就是说，市场收益利率变动引起付息债券市场价格的变动将越来越小。当付息债券的到期日趋向于遥遥无期时，回报终值$FV$对估算现值$PV$的贡献趋向于零，付息债券将转变成永息债券。永息债券没有到期日，期也不偿还本金，其票面价值为$PV = \dfrac{PMT}{i}$。

股票通常可划分成优先股票与普通股票两种类型，优先股票按期支付固定股息，其估值模型与债券的估值模型类似，而普通股票由于按期支付的股息并不固定，可通过金融资产估值公式来估算其现值。普通股票的股利$CF_t$按发行该股票的公司业绩来估算，而转手市场价格$FV$按股票市场的行情来估算。若该普通股票的变化有特定的规律，一般可以取$n = 3$或者更多进行估算，若该普通股票的变化无特定规律，则可以取$n = 1$进行估算，此时金融资产估值公式退化成$PV = \dfrac{CF_1 + FV}{(1 + i)^1}$，而当期收益利率为$i = \dfrac{CF_1}{PV} + \dfrac{FV - PV}{PV}$，即当期收益利率股利收益利率与资本收益利率之和。在简化的情形中，若假设在一段时期内，发行普通股的公司业绩按比例逐步上升，则每期股利总是按小于收益利率$i$的固定增长率$g$呈线性变化，则普通股票的估值公式可简化为$PV = PMT × \sum\limits_{t = 1}^n \left( \dfrac{1 + g}{1 + i} \right)^t + \dfrac{FV}{(1 + i)^n}$，其中$PMT$为此时的实际股利。而当公司业绩趋于平稳时，每期股利趋于保持不变时且固定增长率$g$趋于零，此时普通股票的估值公式可简化为$PV = PMT × \sum\limits_{t = 1}^{n} \dfrac{1}{(1 + i)^t} + \dfrac{FV}{(1 + i)^n}$。
...
> \begin{aligned}
>    &  金融资产估值公式： \\
> PV &= \dfrac{CF_1}{(1 + i)^1} + \dfrac{CF_2}{(1 + i)^2} + \cdots + \dfrac{CF_n}{(1 + i)^n} + \dfrac{FV}{(1 + i)^n} = \sum_{t = 1}^n \dfrac{CF_t}{(1 + i)^t} + \dfrac{FV}{(1 + i)^t} \\
> 
>    &  债券估值公式： \\
> PV &= PMT × \left[ \dfrac{1}{(1 + i)^1} + \dfrac{1}{(1 + i)^2} + \cdots + \dfrac{1}{(1 + i)^n} \right] + \dfrac{FV}{(1 + i)^n} = PMT × \sum_{t = 1}^n \dfrac{1}{(1 + i)^t} + \dfrac{FV}{(1 + i)^n} \\
>    &= PMT × \dfrac{1}{i} \left[ 1 - \dfrac{1}{(1 + i)^n} \right] + \dfrac{FV}{(1 + i)^n} \\
>    &\xlongequal{n \to ∞} \dfrac{PMT}{i} \\
> 
>    &  股票估值公式： \\
> PV &= PMT × \left[ \left( \dfrac{1 + g}{1 + i} \right)^1 + \left( \dfrac{1 + g}{1 + i} \right)^2 + \cdots + \left( \dfrac{1 + g}{1 + i} \right)^n \right] + \dfrac{FV}{(1 + i)^n} = PMT × \sum_{t = 1}^n \left( \dfrac{1 + g}{1 + i} \right)^t + \dfrac{FV}{(1 + i)^n} \\
>    &= PMT × \dfrac{1 + g}{i - g} \left[ 1 - \left( \dfrac{1 + g}{1 + i} \right)^n \right] + \dfrac{FV}{(1 + i)^n} \\
>    &\xlongequal{n \to ∞} PMT × \dfrac{1 + g}{i - g} \\
> \end{aligned}
> $$
> $$
> \begin{aligned}
...
>           &\Downarrow\implies i ≈ \dfrac{PMT + \dfrac{FV - PV}{n}}{\dfrac{FV + PV}{2}} \\
> (1 + x)^n &≈ \dfrac{1}{0 !} + \dfrac{n}{1 !} x + \dfrac{n (n - 1)}{2 !} x^2 + \dfrac{n (n - 1) (n - 2)}{3 !} x^3 + o (x^3) \\ 
>           &\implies i ≈ \dfrac{PMT + \dfrac{FV - PV}{n}}{\dfrac{FV + 2 · PV}{3}} \\
> \end{aligned}
> $$

于2020年02月24日 02:02 +08 在湛江 +08

--------------------------------------------------------------------------------


存款/股票/债券/保险/期货 金融市场/金融监管 股权/债权/所有权/合同 企业/公司 通货膨胀/政府债务 国内生产总值/物价指数 基尼系数 税收/社会保险/失业率 金融危机
