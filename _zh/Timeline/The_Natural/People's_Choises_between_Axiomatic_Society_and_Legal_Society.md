...
Revised:   2020-02-21 04:46:00 +08 @ 中国-广东-湛江 +08
Authors:   璀璨星辰
Resources:
---
...
等额分期存款可通过按期初结算的终值公式来计算每次存款额，若不考虑通货膨胀因素，则终值因子按实际年利率进行复利计算。假设在实际年利率恒为$r = 5\%$的条件下，希望在$n = 3年$后得到存款连本带息的终值为$FV = 10000美元$，则每次存款额为$PMT = \left. FV \middle/ \dfrac{(1 + r)^n - 1}{r} \right. = \left. 10000 \middle/ \dfrac{(1 + 5\%)^3 - 1}{5\%} \right. ≈ 3172.0856美元$，最终累积存款额为$PMT × 3 ≈ 3172.0856 × 3 ≈ 9516.2568美元$。若考虑通货膨胀因素，则每次存款额也即名义每次存款额将随通货膨胀因子而逐步升高，而终值因子按名义年利率进行复利计算。假设在通货膨胀率恒为$π = 3\%$的条件下，名义年利率恒为$i = (1 + r) × (1 + π) - 1 = (1 + 5\%) × (1 + 3\%) = 8.15\%$。

$10000 × (1 + 3\%)^3 ≈ 10927.27美元$。



等额分期贷款有等额本金与等额本息两种偿还方式，在等额本金偿还方式中，每笔还款额的本金相等，而且在往后每笔还款额中尚未偿还余额的利息逐步降低。在等额本息偿还方式中，每笔还款额的本金与尚未偿还余额的利息之和相等，而且在往后每笔还款额中本金逐步升高，尚未偿还余额的利息逐步降低。等额本息偿还方式可通过按期末结算的现值公式计算每笔还款额，假设在年利率为$6\%$也即每月利率为$i = \dfrac{12\%}{12} = 1\%$的条件下，总贷款额为$PV = 10000美元$，共计分3个月偿还，则每期偿还支付额为$PMT = \left. PV \middle/ \dfrac{1 - (1 + i)^{-n}}{i} \right. = \left. 10000 \middle/ \dfrac{1 - (1 + 1\%)^{-3}}{1\%} \right. ≈ 3400.22美元$，总还款额为$PMT × 3 = 3400.22 × 3 = 10200.66美元$为。



| **（不考虑通货膨胀）** |    **等额分期存款** |           (1) + (3) |                     |                 |       (1) × (4) |
| ---------------------: | ------------------: | ------------------: | ------------------: | --------------: | --------------: |
| **（按期初结算）年数** | **实际每次存款**(1) |     **期初存款**(2) |     **期末本息**(3) | **终值因子**(4) | **存款终值**(5) |
|                      1 |           3172.0856 |           3172.0856 |           3330.6899 |          1.1025 |         3497.22 |
|                      2 |           3172.0856 |           6502.7755 |           6827.9143 |          1.0500 |         3465.23 |
|                      3 |           3172.0856 |          10000.0000 |          10500.0000 |          1.0000 |         3172.08 |
|               **总计** |           9516.2568 |                     |                     |          3.1525 |        10000.00 |
|                        |                     |                     |                     |                 |                 |
|   **（考虑通货膨胀）** |    **等额分期存款** |                     |           (1) × (2) |                 |       (3) × (4) |
| **（按期初结算）年数** | **实际每次存款**(1) | **通货膨胀因子**(2) | **名义每次存款**(3) | **终值因子**(4) | **存款终值**(5) |
...
|               **总计** |           9516.2568 |                     |        10098.737509 |                 |    10927.269182 |
|                        |              |              |              |              |              |
| **（等额分期贷款）** | **等额本息方式** | (3) + (4) | | | (1) - (4) |
| **（按期末结算）月数** | **期末余额**(1) | **每次还款**(2) | **偿还利息**(3) | **偿还本金**(4) | **下一期初余额**(5) |
|                      1 |     10000.00 |      3400.22 |       100.00 |      3300.22 |      6699.78 |
|                      2 |      6699.78 |      3400.22 |        67.00 |      3333.22 |      3366.56 |
|                      3 |      3366.56 |      3400.22 |        33.67 |      3366.56 |         0.00 |
|               **总计** |              |     10200.66 |       200.67 |     10000.00 |              |
|  | |  |  |  | |
| **（等额分期贷款）** | **等额本金方式** |  |  | (2) + (3) | (1) - (2) |
| **（按期末结算）月数** | **期末余额**(1) | **偿还本金**(2) | **偿还利息**(3) | **每次还款**(4) | **下一期初余额**(5) |
| 1 | 10000.00 | 3333.33 | 100.00 | 3433.33 | 6666.67 |
| 2 | 6666.67 | 3333.33 | 66.67 | 3400.00 | 3333.33 |
| 3 | 3333.33 | 3333.33 | 33.33 |         3366.67 | 0.00 |
| **总计** |  | 10000.00 | 200.00 |        10200.00 | |
...
估值原理 存款/股票/债券/保险/期货 金融市场/金融监管 股权/债权/所有权/合同 企业/公司 通货膨胀/政府债务 国内生产总值/物价指数 基尼系数 税收/社会保险/失业率 金融危机
