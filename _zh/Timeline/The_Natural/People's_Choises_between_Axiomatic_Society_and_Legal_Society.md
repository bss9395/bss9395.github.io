...
# 事件逻辑
没戏事件，相当于逻辑值必为假的假命题，其逻辑值也即概率为P(𝟘) = 0。
偶然事件，相当于逻辑值模糊不定的命题，其逻辑值也即概率为P(E) ∈ [0, 1]。
必然事件，相当于逻辑值必为真的真命题，其逻辑值也即概率为P(𝟙) = 1。
...
4依附加前提 ⇒ P(E ∨ F) = P(E) + P(F)
 由容斥公理 ⇒ P(E ∨ F) = P(E) + P(F) - P(E ∧ F)
 由等值演算 ⇒ P(E ∧ F) = 0
4由归约公理 ⇒   E ∧ F  = 𝟘
 由等值演算 ⇒ P(E ∧ F) = 0
 由容斥公理 ⇒ P(E ∨ F) = P(E) + P(F) - P(E ∧ F)
4由等值演算 ⇒ P(E ∨ F) = P(E) + P(F)
4根据循环法 ⇒ [P(E ∨ F) = P(E) + P(F)] ↔ [E ∧ F = 𝟘]
5根据恒等性 ⇒ ∀x[[x ∈ F] → [x ∈ F]]
 根据集合论 ⇒ [E ⊆ F] → ∀x[[x ∈ E] → [x ∈ F]]
