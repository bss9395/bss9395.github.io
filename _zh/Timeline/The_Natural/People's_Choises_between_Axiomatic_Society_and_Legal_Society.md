...
从直观上来看，在直线数轴上任意固定一点作为原点，可以将直线数轴对称拆分成方向相反的两条射线，原点即为直线数轴上唯一的中性点。在直线数轴上，任意一个数既代表一个点，也代表由原点指向该点的有向线段，通常约定以数字零代表原点，以数字一代表正方向上单位刻度处的点。任意一个正整数，都可以表示将数字一所代表的有向线段，朝正方向等距叠加该正整数大小多次而成的有向线段。任意一个负整数，都可以表示将数字一所代表的有向线段，朝负方向等距叠加该负整数大小多次而成的有向线段。反之，数字一可以表示将任意一个整数所代表的有向线段，有方向地等距分割该整数大小多次而成的有向线段。因此，任意一个分子与分母均为整数的有理数，都可以表示分子所代表的有向线段，有方向地等距分割分母大小多次而成的有向线段。从直观上来看，将有理数的分子与分母同时乘以一个整数，可以表示将分子所代表的有向线段与分母所代表的有向线段，同时有方向地等距叠加该整数大小多次，叠加前的分子与分母在直线数轴上始终等比例保持相似性，因此有理数的值保持不变。对于任意两个有理数的加法运算，应先将彼此的分母转换成相等的有向线段，可以将两个有理数的分子与分母同时乘以彼此的分母，然后将两个有理数的分子相加，可以表示两个有理数在相同分母所代表的有向线段基础上，有方向地等距叠加两个有理数的分子之和大小多次。对于任意两个有理数的乘法运算，应先将第一个有理数的分子乘以第二个有理数的分子，然后将第一个可通约量的分母乘以第二个可通约量的分母，可以表示第一个有理数的分子所代表的有向线段，有方向地等距叠加第二个有理数的分子大小多次，第一个有理数的分母所代表的有向线段有方向地等距分割第二个有理数的分母大小多次。对于任意两个有理数的减法运算，若这两个有理数相等，则其结果必定为数字零，反之数字零加上任意有理数的结果为该有理数。因此数字零对于减法运算具有特殊性，在减法运算中减去一个有理数，就相当于加上这个有理数关于数字零对称的有理数。对于任意两个有理数的除法运算，若这两个有理数相等，则其结果必定为数字一，反之数字一乘以任意有理数的结果为该有理数。因此数字一对于除法运算具有特殊性，在除法运算中除以一个有理数，就相当于乘以这个有理数关于数字一对称的有理数。显而易见，在直线数轴上的任意一个实数，要么为有理数，要么为无理数。但是以人类现有的认知水平，还无法直接理解无理数的运算过程，因此无理数应先转换成小数形式。有限小数与无限循环小数总是可以表示成有理数，但无限不循环小数不可以表示成有理数，因此有限小数与无限循环小数即为有理数，无限不循环小数即为无理数。无限不循环小数可以借助有限小数经无限逼近过程获得任意精度的近似值，这就等价于无理数可以借助有理数经无限逼近过程获得任意精度的近似值，因此理论上无理数的运算可以借助有理数的运算来理解与定义，这正是数学史上最初建立实数时所采用的古典构造方法。但是任意精度的近似值都必然有误差存在，因而以近似值限定范围的实数，在运算中为保持表示上的一致，就不可避免要对误差进行估算，这会使得看似简单的运算过程变得很繁琐。
