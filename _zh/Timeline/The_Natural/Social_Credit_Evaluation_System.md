...
对于哈希链中的存储机制，信用评价比社会契约稍微简单一些，现考虑发生在两条哈希链之间信用评价。为简化问题，假设主哈希链存储在主节点上，从哈希链存储在从节点上，主节点通过某种方式查询到从节点的信用后，由主节点发起对从节点的信用评价。主节点生成对从节点的信用评价，在单个评分ID中填入主节点的ID，在评分分数中填入对从节点的评分分数，在评分解释中填入对从节点的评分解释，并盖上主哈希链的时间戳。主节点将主哈希链的头部区块附加在信用评价上，再用自己的私钥对这份信用评价进行数字签名，接着将这份信用评价发往从节点。这份信用评价必须先后随机历经至少4个不同的服务器节点，这4个服务器节点必须满足的条件是存储有主哈希链。每个服务器节点将检查信用评价上的时间与本服务器上的时间是否矛盾，并校验主哈希链的头部区块是否一致。再用自己的私钥对这份信用评价单独进行数字签名，每次数字签名都可以采用不同的非对称密钥算法，并将所采用非对称密钥算法附加在数字签名上。从节点在对信用评价进行数字签名后，便将信用评价发回主节点。主节点根据信用评价上数字签名附加的非对称密钥算法，依次验证所有6个服务器的数字签名。如果6个数字签名全都验证成功，就意味着该信用评价被包括主节点在内的5个服务器成功接收，主节点就可以将该信用评价区块加入到主哈希链中，除从节点外的其余节点依次验证完所有数字签名后，也将该信用评价区块加入到主哈希链中。当从节点验证完所有数字签名后，从节点按照同样的方式，将信用评价区块加入到5个存储有从哈希链的服务器节点上。

经过这两次双轮循环多重签名机制，主哈希链与从哈希链将分别成功存储到5个不同的服务器节点上。然后这些服务器节点向网络中所有服务器节点广播这个新加入的信用评价区块，
