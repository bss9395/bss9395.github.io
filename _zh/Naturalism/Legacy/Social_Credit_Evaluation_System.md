...
网络中与哈希链存储相关的节点至少可分为3种类型，即服务器节点/手机客户端/智能身份证，其中服务器节点存储着完整的哈希链，手机客户端至少存储着哈希链的头部区块，智能身份证至少存储着哈希链的头部区块中的实时信用。服务器节点之间必定通过网络进行互相通信，而且服务器节点可以随时加入与退出。手机客户端有可能暂时出现没有网络连接的情况，智能身份证在大多数情况下可以没有网络连接。网络中的每个服务器节点存储着不定数量的实体的哈希链，每个实体的哈希链至少存储在6个服务器节点上。网络中的每个服务器是对等的，不存在统一协调的中心服务器。随着实体接入网络的次数增多，每个实体的哈希链将存储在远远超过6个服务器节点上，哈希链存储在越多的服务器节点上，就越能抵抗黑客攻击。也就是说广义的哈希链就是一个完全分布式的数据库，这个数据库由网络中所有服务器节点组成，并通过冗余数据存储的方式来保证平稳运行。没有统一的中心服务器也就没有协调统一的时间，每个对等的服务器按照自己的时间为哈希链中的区块加盖时间戳。因此时间戳里的时间并不是特别精确的时间，这个时间主要用于校验同一实体的哈希链中前后区块的次序。区块时间戳一定以主节点上的时间为准。

![max-width:480px;](figures/Axiomatic_Human_Society/Bicyclic_Verification_for_Credit_Evaluation.png)

考虑通过服务器节点，在实体ID2对实体ID1的信用评价，假设主节点上存储有实体ID1的哈希链，实体ID1接收到实体ID2的信用评价后，最终将在实体ID1的哈希链中添加信用评价区块。由主节点生成信用评价区块，在单个评分ID中填入ID2，在评分分数中填入ID2对ID1的评分分数，在评分解释中填入ID2对ID1的评分解释，并以主节点上的时间加盖时间戳。主节点将哈希链的头部区块附加在信用评价区块上，再用自己的私钥对这份信用评价进行数字签名，接着将这份信用评价区块发往从节点。这份信用评价区块必须先后随机遍历至少4个不同的服务器节点，这些服务器节点必须满足的条件是存储有ID1哈希链。每个服务器节点将检查信用评价区块上的时间戳与本服务器上的时间是否矛盾，并校验哈希链的头部区块是否一致。再用自己的私钥对这份信用评价单独进行数字签名，每次数字签名都可以采用不同的非对称密钥算法，并将所采用非对称密钥算法附加在数字签名上。从节点在对信用评价进行数字签名后，便将信用评价发回主节点。主节点根据信用评价区块上数字签名附加的非对称密钥算法，验证所有6个服务器的数字签名。如果6个数字签名全都验证成功，就意味着该信用评价区块被包括主节点在内的6个服务器成功接收，主节点就可以将该信用评价区块加入到ID1的哈希链中，其余服务器节点成功验证所有6个数字签名后，也将该信用评价区块加入到ID1的哈希链中。当从节点验证完所有数字签名后，就意味着信用评价区块被成功添加到6个服务器中ID1的哈希链上。此时这6个服务器节点相对于网络中其余存储有ID1哈希链的服务器节点，从某种程度上来说就是权威节点，这6个权威节点向网络广播这份信用评价区块，其余存储有ID1哈希链的服务器节点将这份信用评价添加入ID1哈希链中，从而完成全网的ID1哈希链的更新。

![max-width:480px;](figures/Axiomatic_Human_Society/Bicyclic_Verification_for_Social_Contact.png)

考虑通过服务器节点，在实体ID1/实体ID2/实体ID3之间签订的社会契约，假设ID1主节点上存储有ID1哈希链，ID2主节点上存储有ID2哈希链，ID3主节点上存储有ID3哈希链。实体ID1/实体ID2/实体ID3共同签订的社会契约，最终将在ID1哈希链/ID2哈希链/ID3哈希链中添加社会契约区块。由ID1主节点生成社会契约区块，在多个契约ID数据项中填入ID1/ID2/ID3，在多个实时信用数据项中填入ID1/ID2/ID3的实时信用，在契约内容中填入ID1/ID2/ID3签订的社会契约文本，并以ID1主节点上的时间加盖时间戳。ID1主节点将ID1哈希链头部区块附加在信用评价区块上，在用自己的私钥对这份社会契约区块进行数字签名，但须实体ID1亲手确认无误，接着将这份社会契约区块发往ID1从节点。这份社会契约区块必须先后随机遍历至少4个不同的存储有ID1哈希链的服务器节点，每个ID1服务器节点会检验这份社会契约区块的时间戳与本服务器上的时间是否矛盾，并校验ID1哈希链的头部区块是否一致，再用自己的私钥对这份社会契约单独进行数字签名。ID2主节点接收到来自ID1从节点的这份社会契约区块后，又将ID2哈希链头部区块附加在信用评价区块上，再用自己的私钥对这份社会契约区块进行数字签名，但须实体ID2亲手确认无误，接着将这份社会契约区块发往ID2从节点。这份社会契约区块必须先后随机遍历至少4个不同的存储有ID2哈希链的服务器节点，每个ID2服务器节点会检验这份社会契约区块的时间戳与本服务器上的时间是否矛盾，并检验ID2哈希链的头部区块是否一致，再用自己的私钥对这份社会契约单独进行数字签名。同理，ID3主节点接受到来自ID2从节点的这份社会契约区块后，将遵循同样的流程。当ID3从节点完成最后一次数字签名后，便将这份社会契约区块发回ID1主节点。ID1主节点将验证所有6×3=18个数字签名，如果所有数字签名验证成功，就意味着这份社会契约区块成功被6个不同的ID1服务器/6个不同的ID2服务器/6个不同的ID3服务器成功接收，ID1主节点便将这份社会契约区块添加到ID1哈希链上。同理，其余服务器节点再成功验证所有数字签名后，便将这份社会契约区块添加到本服务器的哈希链上。然后这6个不同ID1/ID2/ID3服务器节点，就是ID1/ID2/ID3哈希链的权威结点。由这些权威服务器节点向全网进行广播，使得存储有ID1/ID2/ID3哈希链的服务器节点，将这份社会契约区块添加到ID1/ID2/ID3哈希链上，从而完成全网的ID1/ID2/ID3哈希链的更新。

考虑手机客户端与主节点的数据同步，假设存储有ID1哈希链头部区块的手机客户端成功连接到被称为主节点的服务器节点，但发现主节点上没有存储着ID1哈希链。主节点向全网的服务器节点查询ID1哈希链，全网所有的ID1服务器节点都将返回ID1哈希链的头部区块。假设所有返回的ID1哈希链头部区块中，有占多数的头部区块是相同的，但也有占少数的头部区块是不同的，如果占多数的头部区块与占少数的头部区块的比例达到6:1以上，那么就认为占多数的头部区块是可信的，而认为占少数的头部区块是不可信的。主节点将下载占多数的头部区块的ID1哈希链，从而完成主节点对ID1哈希链的数据同步。主节点将丢弃占少数的区块，同时向社会信用评价系统的管理员报告问题。当手机客户端与主节点有网络连接时，手机客户端将信用评价与社会契约等等数据全部发送给主节点，由主节点完成哈希链的存储，手机客户端仅在查询时保留一部分缓存数据。

考虑手机客户端切断网络连接时的数据存储，此时要求手机具有某些近距离通信功能，比如近距离无线电通信技术NFC。假设ID1手机上存储有ID1哈希链头部区块，ID2手机上存储有ID2哈希链头部区块，ID3手机上存储有ID3哈希链头部区块。若实体ID2对实体ID1进行信用评价，可由ID2手机生成信用评价区块，在单个评分ID数据项中填入ID2，在评分分数数据项中填入ID2对ID1的评分分数，在评分的解释数据项中填入ID2对ID1的评分解释，并添加ID2手机上的时间。这份信用评价区块保存在ID2手机上，当ID2手机有网络连接时，ID2手机将这份信用评价区块发送给ID2主节点，ID2主节点通过查询并同步ID1哈希链的方式成为ID1主节点，再由ID1主节点经过双轮循环验证机制，从而在全网将这份信用评价区块添加入ID2哈希链当中。若实体ID1/实体ID2/实体ID3共同签订社会契约，由ID1手机生成社会契约区块，在多个契约ID数据项填入ID1/ID2/ID3，在多个实时信用数据项填入ID1/ID2/ID3的信用，在契约的内容数据项填入ID1/ID2/ID3共同签订的契约文本内容。由ID1手机将这份社会契约发送给ID2手机与ID3手机，实体ID1/ID2/ID3在各自的手机上通过指纹等等验证方式确认无误后，ID1/ID2/ID3手机分别对这份社会契约区块进行数字签名并存档，用于联网时主节点对社会契约的自动确认。等到ID1/ID2/ID3手机与ID1/ID2/ID3主节点完成数据同步后，再由ID1主节点经过双轮循环验证机制，从而在全网将这份信用评价区块分别添加入ID1/ID2/ID3哈希链中。对于社会契约，不存在时间次序的问题。但对于信用评价，因为实时信用的更新由原来的实时信用与评分分数共同决定，所以有必要考虑时间次序问题。一种较好解决方案是，按照信用评价区块上的区块时间戳次序对实时信用进行更新，无论评分实体所看到的被评分实体的实时信用如何。另一种可采用的解决方案是，按照信用评价区块的达成时间对实时信用进行更新，但因为区块时间戳总是落后于达成时间，因此实时信用的更新是按达成时间计算的实时信用的平均值，不会出现哈希链分叉问题。社会信用评价系统采用第一种解决方案，因为信用评分是以评分实体自身为基准，实际上不需要知道被评分实体的实时信用。

考虑智能身份证的数据存储。智能身份证至少存储有实体的ID与实时信用，
